...
**Why this file exists.** Created at the session-247 reconcile. The base
`2026-h1-status.md` stood at **189,574 B** — 7,034 B under R4's ~192 KiB split
trigger — while the Active-TODOs compliance trim needed to archive **21,958 B** of full
originals under R4's move-never-drop rule. Appending them would have carried the base to
**211,532 B**, past the trigger (though still inside the 256 KB hard cap), so the
split ran FIRST and the trim landed against a base with room. **No content lost:** every
section is preserved verbatim and exactly once across the chain, verified by exact
multiset equality of the `##` headings AND the `###` blocks before and after — not by a
byte-sum estimate.
