# STATUS.md rotation archive — 2026 H1, earlier window (`-b` continuation)
...
~192 KB, start a `-b` continuation file"). It holds the **earlier** half of the
2026-h1 rotation archive — split out of `2026-h1-status.md` on 2026-06-26
(session 80) when the combined file crossed the ~192 KB bar. Same two sections
per R4 (rotated Current Focus blocks + rotated Recent Decisions rows, newest at
top, each tagged with its rotation date); every block/row preserved verbatim,
no content lost. Tier-3: grep + windowed reads only.
