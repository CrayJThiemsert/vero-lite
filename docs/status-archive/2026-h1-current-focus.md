# STATUS.md — archived Current Focus blocks (2026 H1)

Older `## Current Focus` session blocks rotated out of `docs/STATUS.md` on 2026-06-10 (session 51) so the always-read Tier-1 STATUS stays small — it had grown to 393 KB / >25k tokens and could no longer be read in full by the Read tool (a single frontmatter line was 48 KB). Newest blocks remain in STATUS.md; these are Session 46 and earlier. Full record also in git history (Tier 3).
