...
> **Sibling chain (letters ascend with time; the base file holds the RECENT window):** [`2026-h1b-status.md`](2026-h1b-status.md) (2026-05-10 → 2026-06-09) → [`2026-h1c-status.md`](2026-h1c-status.md) → [`2026-h1d-status.md`](2026-h1d-status.md) → [`2026-h1e-status.md`](2026-h1e-status.md) → [`2026-h1f-status.md`](2026-h1f-status.md) → [`2026-h1g-status.md`](2026-h1g-status.md) (this file) → [`2026-h1h-status.md`](2026-h1h-status.md) → [`2026-h1i-status.md`](2026-h1i-status.md) → [`2026-h1-status.md`](2026-h1-status.md) (base, newest — rotations append THERE). The separate `2026-h1-current-focus.md` (sessions ≤46, ratified as-is) is a Current-Focus-only artifact predating this chain.
...
**Why this file exists.** Created at the session-193 reconcile. The base
`2026-h1-status.md` stood at **194,232 B** — 2,376 B under R4's ~192 KiB split
trigger — and the s193 reconcile had to evict the session-186→187 Current-Focus block
(~10,661 B), which would have carried the base to roughly 205 KB, past the trigger.
Per R4's continuation rule the OLDER content spills into the next letter, so the
sessions-142→171 reconcile sections moved here and the base kept its recent window.

**Split arithmetic, recorded so a later reader can check it rather than trust it.**
Base before: 194,232 B = 2,129 B header + 148,017 B (the sections now in this file)
+ 44,086 B (retained). Base after: 46,215 B. This file: this header + 148,017 B.
**No content lost** — the byte sums reconcile exactly, and the section list was
compared for equality before and after, not estimated.

**Structural note, inherited from the base file and still honest.** R4 describes an
archive as TWO sections — rotated Current Focus blocks and rotated Recent Decisions
rows, *newest at top*. That is not the shape on disk: the corpus drifted into **one
section per reconcile, appended at the bottom**. This split preserves the drifted
shape rather than silently rewriting history to match the spec; reconciling R4's text
with the real convention remains separate work, deliberately not done here.
