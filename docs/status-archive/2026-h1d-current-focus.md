...
> 🔒 **CLOSED TO APPENDS session 281 (2026-09-06).** The live append target is now [`2026-h1e-current-focus.md`](2026-h1e-current-focus.md). This file stopped at **189,622 B** with **6,986 B** of headroom under R4's **196,608-byte split trigger**; session 281's rotation is **3,269 B**, which would have taken it to 192,891 B — essentially where `h1c` closed (192,607 B). Per R4 the continuation is started **before** the bar is crossed, not after. It was the live target from session 227 (2026-08-13) until then, and holds sessions 227 → 280.
