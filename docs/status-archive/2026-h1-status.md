...
<!-- rotated 2026-08-12, session 223 (STATUS reconcile — eight PRs behind, #1126–#1133) -->

Three Recent Decisions rows rotated out when session 223's two rows and session 222's
continuation row entered the last-10 window.
...
### Recent-Decisions row — s219 (PLAN-0103 Steps 2 + 3 SHIPPED; the `isPublished()` consumer census corrected to 9)

Rotated out when session 225's row entered the last-10 window.

### Active-TODO rows — two spent `[x]` rows from s221 (energy's live data migration; PLAN-0103's stale self-citations)

Rotated under R2's ratified Active-TODO rule — `[x]` items older than the session window leave STATUS, and git history plus the artifacts they cite hold them. Appended to this base rather than to the Current-Focus chain because they are Active-TODO rows, not Current-Focus blocks (the "RD → base, CF → newest" convention puts all non-CF content here).
