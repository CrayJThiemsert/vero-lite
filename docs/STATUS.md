...
current_actor: code
blocked_on: "NOTHING — all three Cray calls RULED s270 (fl-21/fl-22 = decide from the standing principle, PLAN-0117 SD-7; Thai corpus = gap ACCEPTED, DECISION.md §5-RULED; human half = waits on a 20-pair re-run, same block). Two follow-ons carry their own gates: the re-run needs a typed §8 go PLUS a timeout fix, and the s269+s270 STATUS reconcile is owed (range a0b743b -> HEAD)."
next_action: "PLAN-0118 Step 4 (runner + recording pass-through client + AC-3 scenario test at tests/benchmark/ — singular, flat). All four s268/s269 orphans are now rehomed: AC-1(d) (s269), biomass_boiler provenance + §5a's call-site count + the fl-21/fl-22 design (all s270)."
head_commit: a0b743b
...
> follows.** Full ruling incl. the residual: `docs/plans/0117-*.md` **SD-7**.
>
...
- [ ] **🆕 Subagent write guards → settings-level dispatcher (s272 PR).** s271's nested-shape fix made the agents LOAD (3/3 spawn ✓), not their hooks RUN: s272, new conversation, instrumented the guard script → **0 invocations** while `goal-evaluator`'s allowed `goal.json` write succeeded. Frontmatter hooks are inert in this harness; settings hooks fire for subagent calls and carry `agent_id` + `agent_type` (read off `pretooluse_git_deny`'s own deny text). Fix: `pretooluse_subagent_write_dispatch.py` on PreToolUse `Write|Edit`, routing by `agent_type` to the three **unchanged** guards; wiring pinned both ways + §8 scenario test. ⚠️ **Live witness owed by the next conversation** (settings hooks snapshot at session start): a legitimate `goal-evaluator` dispatch must still write `goal.json` (no false deny); a forbidden write denied if a model will attempt one (Opus refuses). ADR-0018 / PLAN-0034 marker = Cowork dispatch. [Lesson #0057](lessons/0057-a-perfect-correlation-is-not-a-cause-ask-when-it-changed.md) "Resolution, part 2".
- [ ] **🆕 NOW THE LIVE NEXT ACTION — the machine half of the selection criterion.** Unblocked s268 by the verdict above. Of the two positions whose next consumer is **code**, the NL lane already runs its real consumer (`answer_question`); **intake extraction has no benchmark of any kind** (measured s268 — five suites under `benchmarks/`, zero grep hits for `intake`, against a shipped seam at `services/engine/llm/intake.py`). ✅ **Both follow-ons RULED s270, so Cray's ordering is discharged:** the **human half** is unblocked by the existence test but now waits on a 20-pair re-run (§8 go + timeout fix), and the **Thai corpus** got its own decision instead of inheriting a spent hold — gap ACCEPTED. Both at `benchmarks/model_compare/DECISION.md` **§5-RULED**; see the Thai-prose row below. **Read:** `benchmarks/model_compare/DECISION.md` §5a-RESULT.
- [ ] **🆕 TWO of three s268 findings still live ONLY in a gitignored handoff.** ✅ **(i) CLOSED s269 — rehomed into PLAN-0118 AC-1(d) and enforced.** The criterion is now *same reading, same unit*; the `thr/3..thr*3` window it replaced was measured a **false negative** (it excluded `rm-02`'s 0.6 kPa clean-bag reading while every case it did admit was a true positive — 4 admitted, 5 qualify). Each case declares `same_unit_distractors` and `tests/benchmark/test_intake_extraction_gold.py` verifies the declaration against the description, so a wrong one reddens. ✅ **(ii) CLOSED s270 — `biomass_boiler` RATIFIED** (Cray, typed, 2026-09-02) as **PLAN-0118 SD-6**, which records the measured price of the declined veto (domains 3→2 fails Step 2's ≥3; the `stated:true × discordant` cell emptied; `inj-03` lost with `bo-01`). `gold.yaml`'s `domains:` block now carries **PROVENANCE** per domain. ✅ **(iii) CLOSED s270** — re-counted: **three** production call sites, now **named** in `DECISION.md` §5a, corrected in place with a `was an error` marker; all three pass no argument, so DO-NOT-WIRE is unaffected. **Read:** `docs/plans/0118-intake-extraction-benchmark.md`.
...
- [x] **✅ RULED s270 — Thai prose quality is an ACCEPTED, RECORDED GAP:** **no model choice may be defended on it today.** 🔴 Two stale claims cleared with it — (a) the "Thai-brief rubric" said to **gate** this is §5a's *gate-advisory* rubric, already **moot** per §5a-RESULT (*"do not spend a panel, a corpus, or a §8 go"*): a dead gate held a live question; (b) "the **ratified** Thai-corpus option" was **never ratified** — searched s270 across all `docs/` incl. `plans/done/` + all twelve `status-archive/` files, the gitignored FDE carrier doc and every handoff: **zero hits**. Withdrawn. 🔴 **Human half also ruled — waits for a 20-pair re-run:** today's dumps give **PAIRABLE_N = 9 of 20**, and the missing eleven are exactly qwen's B2 failures, so the survivors are biased toward what qwen could finish. Needs a typed §8 go **and** a timeout fix first (qwen p95 120.1 s sits *on* the client timeout). **Read:** `benchmarks/model_compare/DECISION.md` **§5-RULED**.
