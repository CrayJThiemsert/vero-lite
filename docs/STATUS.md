...
head_commit: 9160f4f
recent_commits: [9160f4f, 866e09f, 52832bc, a2e2d0e, 289a003, 03c3d45, e5e4f09, fee2ed2, f2731be, 9abbfa4]
---
...
|------|----------|-----------|
| 2026-08-10 | **s218 cont. — ALL EIGHT PLAN-0103 slots RULED (Cray, typed), and SD-1 spawned ADR-0037 `Proposed` (#1104).** 🔴 **SD-1 OVERRULED both the drafter and Code's R2 concurrence** — both read ADR-0036 D5's "PLAN's to finalize" grant as sufficient; Cray ruled a **new ADR** is required. The ruling is right: a DB on a public system changes what personal data it holds and what erasure can be promised — legal consequence, outliving the PLAN, needing attribution. Two reviewers agreeing was not independent evidence. **ADR-0037** became the home for two findings recorded nowhere: **ADR-0035 D6 is LLM-route-scoped** (case text falls outside it) and a case driving a governed run enters the **audit chain, whose erasure the RoPA says cannot be promised** — whether text is copied there is marked **UNVERIFIED** and made a pre-bring-up measurement (D2.7), not asserted. Other rulings: bring-up **procurement→fleet**; fleet `A,C,F,H,I,J`; procurement `G,F` (Tab H **storage**-blocked, not persona-blocked); persona picker; seed **and** visitor path; Tab E stays in dev; bilingual + translated roles. ⚠️ ADR-0037 gates **fleet's half only** — Step 4's gate map names what proceeds regardless | `9160f4f` (head_commit) / [#1104](https://github.com/CrayJThiemsert/vero-lite/pull/1104) / `docs/adr/0037-published-system-data-persistence-posture.md` |
