---
last_updated: 2026-08-15T03:04:59+07:00
session: 232
current_batch: "s232 fleet bring-up push — seven PRs MERGED (#1170–#1176), 0 open. Grounding REFUTED the inherited \"one artifact left\" framing: three more gates found, five closed, six Cray rulings folded."
current_actor: code
blocked_on: "NOTHING blocks repo work. Fleet's bring-up gates on PLAN-0103 AC-11's RoPA (Cray's), PLAN-0106 SD-1..SD-3 (Cray's), fleet's typed §8 go, 2 Cloudflare artifacts and 4 host secrets."
next_action: "Cray's call — rule PLAN-0106 SD-1..SD-3 (unblocks its Step 0), or author fleet's RoPA; both gate the bring-up."
head_commit: 8b2fc14
recent_commits: [8b2fc14, ec4ac0f, 334d797, 480df33, f2e037a, c764869, 715d063, dbb799d, 24c5676, 1100b7c]
---
...
> **Session 232, 2026-08-15 (head_commit `b2fe45e` → `8b2fc14`) — seven PRs
> MERGED (#1170–#1176), 0 open. A next-work ranking became a fleet bring-up
> push: grounding REFUTED the inherited "blocked on ONE artifact" framing and
> found three more gates. Cray ruled six times; five gates closed.**
>
> 🔴 **PLAN-0105 claimed a factoring that did not exist — `was an error`.** Its
> Out of Scope AND its closeout both said the sweep's per-case deletion unit was
> "factored so a future DSR path can call it". There was none — the work was
> inline in `sweep`'s loop, and `sweep` accepts no case identifier. EXTRACTING
> `delete_case` (#1171) made the ARCHIVED plan true of the tree rather than
> saying less; failure semantics RULED **(b)** — roll back, re-raise.
>
> 🔴 **Three artifacts described one fact and only ONE was right.** Fleet's
> Operate seed shipped s221, yet `published.env`'s comment and the profile
> README still said it was unbuilt and that flipping the flag *"would be a no-op
> that reads like a fix"* — `main.py`'s comment was right **because it was
> edited alongside the code**. ⚠️ **Prose has no consumer:** ruff, mypy and 4093
> tests stayed silent eleven sessions. Flag ARMED (#1170); README fixed (#1173).
>
> ⚠️ **`GET /api/cases` is unauthenticated and unfiltered**
> (`services/api/routers/cases.py:248-278` returns `description` for every row)
> and `^/api/cases$` is on fleet's allowlist — cloudflared matches PATH, not
> METHOD. **RULED INTENDED** (Cray, typed). 🔴 **Record the PREMISE, not the
> word:** legitimate **because the data is synthetic** — the premise going false
> is what makes this stale, not the word "intended". Carried as a **recipients**
> fact, not only a disclosure one: `ropa-change-statement-fleet.md` §4(c).
>
> 🔴 **ADR-0037 D2.4 was an obligation with NO OWNER for three sessions** — the
> ADR said "mechanics belong to the owning PLAN" and no PLAN took it; no STATUS
> row, no test, no checklist held it. Found only by walking D2's obligations one
> at a time. Now owned by **PLAN-0106** (#1174) and RULED **(ก)**: fleet gets
> its OWN in-app disclosure, never a widening of the D6 banner.
>
> ⚠️ **An empty database is invisible until a visitor hits it.** `/health` is
> pure liveness and never touches Postgres, and `cloudflared` gates only on
> `service_healthy` — the tunnel OPENS on a system whose visitor case path fails
> on the first write. RULED **operator step + make the skip LEGIBLE**, not a
> compose `migrate` service; shipped as a boot-log line + 3 scenario tests (#1176).
>
> ✅ **Also RULED and folded in (#1174, #1175):** ADR-0037 **D4/OQ-2 = (a)
> text-by-reference** · recorder free text = **(i)**, its own RoPA line, plainly
> non-erasable · ⚠️ **OQ-1 had been ruled since s231 while the ADR's text still
> read OPEN** — retroactive drift, now closed. ✅ **`deploy.py` does NOT block
> the bring-up** — it is the REDEPLOY tool; procurement went live with plain
> `docker compose up -d`, as fleet's README prescribes (corrects an in-session claim).
>
> ⚠️ **What this does NOT mean.** Fleet's bring-up is still gated: **PLAN-0103
> AC-11's RoPA (Cray's)**, **ADR-0037 D2.4 via PLAN-0106** (Step 0 blocked on
> SD-1..SD-3), fleet's **own typed §8 go**, the two Cloudflare dashboard
> artifacts, and four host secrets. **Requester identification for the DSR path
> remains genuinely undesigned.**
...
|------|----------|-----------|
| 2026-08-15 | **s232 — seven PRs MERGED (#1170–#1176); the inherited "fleet blocks on ONE artifact" framing was REFUTED — three more gates existed, five closed.** Six typed Cray rulings folded: ADR-0037 D4/OQ-2 = **(a) text-by-reference** · recorder free text = **(i)** · D2.4 = **(ก)** fleet's OWN disclosure → **PLAN-0106** · the unauthenticated case list is **INTENDED because the data is synthetic** · schema migration = operator step + legible skip. 🔴 **PLAN-0105's archived text claimed a `delete_case` factoring that did not exist** — EXTRACTED, not re-worded. | `8b2fc14` (head_commit) / [#1171](https://github.com/CrayJThiemsert/vero-lite/pull/1171) / [#1174](https://github.com/CrayJThiemsert/vero-lite/pull/1174) / [#1176](https://github.com/CrayJThiemsert/vero-lite/pull/1176) / `docs/plans/0106-fleet-case-persistence-disclosure.md` |
...
- [ ] **🆕 PLAN-0106 — fleet's OWN in-app case-persistence disclosure, ADR-0037 D2.4's owner. CREATED s232 (#1174), `Status: Draft`. A fleet bring-up PRECONDITION.** 🔴 **D2.4 sat with NO OWNER for three sessions** — the ADR said mechanics belong to the owning PLAN and none took it; no STATUS row, no test, no checklist held it. ✅ **RULED (ก)** (Cray, typed, s232): fleet gets its own disclosure, **never a widening of the D6 banner**. ⚠️ **Step 0 is BLOCKED on Cray ruling SD-1 (wording), SD-2 (placement), SD-3 (the D6 "read only by the operator" clause).** **Read the PLAN, never a restatement:** `docs/plans/0106-fleet-case-persistence-disclosure.md`.
- [x] **PLAN-0105 — in-app 90-day retention for fleet's visitor case data: CLOSED s231, COMPLETE 11/11 and ARCHIVED (#1160–#1167).** Four SD slots RULED (Cray, typed, 2026-08-14) and folded in — **read them in the PLAN's §Surfaced decisions, never a restatement:** `docs/plans/done/0105-fleet-case-retention-in-app-deletion.md`. 🔴 **Do not re-derive: the declared deletion order deleted `repair_case_quote` BEFORE its composite-FK child `repair_case_accepted_quote`** — `ForeignKeyViolation` on every case that had accepted a quote, swallowed by the fail-soft and **retried forever** while unit tests stayed green; **AC-5 checks membership, not order**, and only the Step-6 scenario saw it (`was an error`). 🔴 **CORRECTED s232 (#1171), `was an error`: the archived PLAN claimed a factoring that did not exist** — its Out of Scope AND its closeout both said the per-case deletion unit was *"factored so a future DSR path can call it"*, while the work was inline in `sweep`'s loop and `sweep` accepts no case identifier. Fixed by **EXTRACTING `delete_case()`** (`services/db/repair_case_retention.py`), so the archive became true of the tree rather than saying less; failure semantics **RULED (b)** (Cray, typed, s232) — the unit rolls back its own partial work and re-raises. ⚠️ **What `Complete` does NOT mean:** **PLAN-0103 AC-11's RoPA is still Cray's**, the **DSR-on-request path is still undefined — requester identification in particular is genuinely undesigned** (`delete_case()` is now the callable unit, which is the mechanism half only), and **fleet's bring-up still needs its own typed §8 go**.
