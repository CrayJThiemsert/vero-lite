...
- [ ] **🆕 THREE subagents absent — CAUSE FOUND + FIXED s271** (`goal-evaluator`, `plan-drafter`, `status-scribe`): their frontmatter `hooks:` block was **flat** (never the documented shape); client 2.1.247 silently discarded it, **2.1.255 refuses the agent** (fail-closed). Rewritten to the nested shape (s271 PR); live after hot-reload: **3/3 spawn ✓**. ⚠️ **But the write-deny guard still did NOT fire** — `goal-evaluator`'s Write outside `goal.json` succeeded, while the script run directly denies → the harness skipped the frontmatter hook. So the guards have **never** run in-harness. NEXT (new conversation, post-merge): repeat the probe, read = **deny**; if it still writes, move the guards to `settings.json` dispatching on `agent_type`. ADR-0018 / PLAN-0034 marker = a later Cowork dispatch. Until verified, `/goal` with **`check` only**. [Lesson #0057](lessons/0057-a-perfect-correlation-is-not-a-cause-ask-when-it-changed.md) "Resolution".
