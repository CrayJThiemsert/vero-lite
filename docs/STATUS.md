...
- [x] **`CLAUDE.md` §3 named the code generator as the moat — CORRECTED s202 (#1020), Cowork-drafted per §6 convention.** §3 now leads with ADR-0032 D6's `monitor→decide→approve→act` identity and names the runtime procedure spine as the primitive; codegen is **rescoped, not denied** (only `energy` + `core` emit committed code). `docs/conventions/glossary.md` carried the same framing and moved with it. _[Corrected s202, `was an error` — **this row's batched-in "§1's 'SME' wording" half HAS NO REFERENT and is struck**: the string `SME` has never existed in `CLAUDE.md` (`git log -S "SME" -- CLAUDE.md` returns **zero** commits), and §1 in fact reads "2 **enterprise** design partners". The s197 Current Focus block records the same mistaken premise; both rows are corrected once, here. **Cray's actual s197 point is untouched and needs no edit** — the partners are mid-size regulated operators, which is what §1 already says. Scope is therefore §3 alone.]_ _[Flipped s203 — the row survived the s202 reconcile as `[ ]` even though #1020 had already discharged it. Its stale "other five" figure had also propagated into two `code_generator.py` comments; both were fixed in the same s203 pass, one as "six" (the emitter count is fixed at 7) and one de-numbered (how many outputs are gitignored is namespace-dependent, so any fixed number re-stales).]_
