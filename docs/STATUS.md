...
- [ ] **🆕 Two synthetic cases were left on the published demo by the s256 walk, one with a live gate.** `case-8a25399bd734` (฿12,000, run `@41bb7835…` **parked at a real single-proposal gate** — a visitor can resolve it, which is the demo's own beat) and `case-596c0244e638` (฿4,500, run completed). Both labelled `s256 live walk — ข้อมูลสังเคราะห์`. **Cray's call whether to clear them** — `deploy/published/oct-fleet-maintenance/DEMO-RESET.md` is the procedure; **not run**, because demo content is not a deploy step.
