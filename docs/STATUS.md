...
- [ ] **🆕 THREE subagents absent from the harness registry** (`goal-evaluator`, `plan-drafter`, `status-scribe`). 🔴 **DO NOT edit `.claude/agents/*.md`** — s270 proved the files are not the cause; it is client-side. **Restart the session first.** Until they return, declare `/goal` with **`check` criteria only**. Full diagnosis + the four refuted hypotheses: [Lesson #0057](lessons/0057-a-perfect-correlation-is-not-a-cause-ask-when-it-changed.md).
