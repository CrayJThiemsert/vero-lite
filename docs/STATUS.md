...
- [ ] **🆕 FIRST ACTION, ruled by Cray s267 — run the GATE EXISTENCE TEST, then the machine half.** Offline, ~70 min of Cray's time, nothing blocks it. The test's own spec now lives with the ruling it resolves (`DECISION.md` §5a post-ruling caveat), because §5a was measured to have an **empty-reward-set hole**: the sidecar's first reason already states the causal link verbatim, so the ruled rubric rewards paraphrasing what is on screen. A negative outcome **costs nothing** — the live arm has never been wired. 🔴 **Cray's ruled order after it:** (1) the **machine half** — intake is the gap and has **no benchmark of any kind** (measured s268, zero grep hits); (2) the **human half** only after the existence test, scoped **down** by specialists posing questions for Cray to rule on; (3) the **Thai corpus is deliberately NOT started** — a decision, not an omission, since the test may cut the position it would serve. **Read:** `benchmarks/model_compare/DECISION.md` §5a.
...
> **Immediate next action is the GATE EXISTENCE TEST** — see the first Active TODO above. The items below are the long-horizon register and none of them gates it.
