...
session: 51
current_batch: Session 51 — **ADR-0018 "Axis-B Verification Loop" RATIFIED Accepted (#242, head_commit `1be60f7`, `docs(adr):`) — the deferred Axis-B verification-loop track is no longer deferred; it is now Accepted and entering its build phase.** ADR-0018 opens harness-review track 2 (the evaluator loop) on top of the at-frontier Axis-A governance layer: a `/goal` Stop-hook gate + a `goal-evaluator` subagent. Decisions: D1 hybrid deterministic-check + LLM-judge; D2 `.claude/state/goal.json` run-goal artifact; D3 a 4th subagent sibling that REFUTES-not-blesses; D4 a `_goal_gate.py` inside `stop_continuation.py`, **FAIL-OPEN**; D5 a session-Stop warn-only v1; D6 the structural reasoning-blindness rationale; D7 formalize+augment the manual AC ritual. **SD-1 resolved = narrowed Write** (the evaluator's Write is hook-narrowed to `goal.json` only). Lineage: PR #241 added the ADR `Proposed` (`5f8073c`, `docs(adr):`) → **#242 ratified it Proposed→Accepted (Cray, session 51)** + carries this T4 STATUS reconcile (the ADR §"Required follow-on" T4 task). head_commit = `1be60f7` (the ratification `docs(adr):` commit = the newest *substantive* commit per `lint_status`; the `docs(status):` reconcile commit does NOT count). **NEXT = T2:** Code dispatches the Axis-B **build PLAN** to Cowork (ADR-009 D1) covering `_goal_gate.py` + the `stop_continuation.py` insertion, the `goal.json` schema + tests, the `/goal` command, the `goal-evaluator` agent + the SD-1 write-deny hook, Telegram wiring, and a verification-rigor case-coverage matrix; after draft+ratify → T3 (autonomy-triggers V-row) → build. OQ-8 (plugin packaging, MS-S1 local evaluator, blocking-mode promotion, PR-merge gating, auto-declared goals) and VX-1..3 stay non-binding / verify-at-execution — not blocking. AI-assisted (Claude Code, session 51), no `Co-Authored-By`. · **Prior (session 51 earlier) — ADR-0017 OQ-B RESOLVED; skills-as-memory-tier arc's last open thread CLOSED (#239, `c512cf9`, `docs(adr):`).** Empirical, restart-confirmed (~99%): on a same-bare-name collision the GLOBAL/user skill (`C:\Users\crayj\.claude\skills\`) WINS over the project skill — the OPPOSITE of ADR-0017 D7's "project-local wins" premise; WSL `~/.claude/skills/` not scanned; plugin skills namespaced. Recorded via ADR-0017 "D7 Errata (2026-06-10)" subsection + OQ-B Open→RESOLVED (errata applied with Cray's per-diff approval through the G1 Accepted-ADR gate), new `docs/lessons/0022-skill-loader-precedence.md`, and a fill of the `docs/runbooks/memory-architecture.md` §"Skill Conventions" OQ-B placeholder. D7 authority rule (global/plugin skills must not encode project-binding rules) UNCHANGED + reinforced; only loader tie-break mechanics + an incidental `eli-cray` example (a command, not a global skill) corrected. Probe host-state cleaned up; tree clean. AI-assisted (Code, session 51), no `Co-Authored-By`. Prior: Session 49 — **ADR-0017 "Skills as a memory tier" is ACCEPTED + the skills-as-memory-tier governance arc is COMPLETE (#236 + #237, head_commit `8b18b3a`, `docs(constitution):`).** #236 added the ADR (Cowork-drafted, Code-reviewed via ADR-009 D3) `Proposed`; #237 ratified it (Proposed→Accepted) + applied the alignment (T1–T5): `.claude/skills/` is now **Tier 2.6** in the memory model (`CLAUDE.md` §4 + memory-architecture runbook), git-tracked, auto-loaded by description match; the **D5 knowledge-placement decision rule** (binding→CLAUDE.md; durable learning→lessons; canonical reference→conventions/runbooks; task-triggered how-to→a Skill) + **D7 skill-authoring conventions** codified in the runbook; §1 gained the **D6** derived-precedence line (2.5+2.6 carry no independent precedence; canonical wins); §10 skills row cites ADR-0017. This is T6 (STATUS reconcile) of the ADR-0017 follow-on. **Lineage:** PR #234 (`471bcb5`) added the `.claude/skills/` *mechanism* → #236 the *ADR* → #237 the *alignment*. Prior (the #234 mechanism turn): the always-loaded constitution shrank `206→193 lines / 2050→1908 words` by extracting git mechanics + the Tier-2 Code-ops procedure into TWO new on-demand project skills under `.claude/skills/` (**`git-workflow`** + **`code-operational-policy`**), with **all binding rules retained in `CLAUDE.md`** (only step-by-step procedure moved). Establishes `.claude/skills/` as the project's on-demand procedure layer (Anthropic's Agent-Skills pattern: a bloated always-on CLAUDE.md causes ignored rules → procedure belongs in on-demand skills). **Genesis = the two-axis harness-engineering review (2026-06-09):** Axis A (governance/safety) = vero-lite at the frontier (deterministic-hooks + Sonnet-classifier hybrid independently mirrors Anthropic *auto mode*; `chain-cap=8` matches Claude Code's own default); Axis B (task-completion/verification — the evaluator loop) = thin, deferred as a separate higher-leverage track. #234 = the low-risk FIRST move Cray chose. **Follow-up DISPATCHED to Cowork:** draft ADR-017 (likely) "Skills as a memory tier" — handoff (gitignored) `.claude/handoffs/session-49/2026-06-09-2140-code-skills-memory-tier-adr-dispatch.md` (8 OQs: tier placement, `for_llm/` overlap, canonical-vs-derived rule, authoring ownership, knowledge-placement decision rule, precedence ladder, conventions, migration backlog); Code commits the resulting `Proposed` ADR per ADR-009 D2. This block = the session-49 #234 reconcile (head_commit `471bcb5` — newest substantive per `lint_status`; `docs(constitution):` IS substantive; the #234 merge commit `d556421` is lint-excluded). · **Prior context (session 49 earlier — PLAN-0020 Phase 1 (OFFLINE) is COMPLETE + merged (#232, `8324cba`, `feat(engine):`))** — two complementary, INDEPENDENT product-code changes addressing the session-48 PLAN-0019 Part B aquaculture over-naming finding: **fix #2 — deterministic `affected_entities` override (PRODUCT path)** (`action_step.py` `_compose_action` now sources `affected_entities` from the single loop `event` entity via a new `_loop_entity_ref`, NOT the model's guess — mirrors the existing `step.handler` override; closes the envelope over-naming metadata/UX leak; the ADR-007 D2 `RecommendedAction` envelope CLASS is UNCHANGED, one field sourced deterministically) · **the aqua prompt nudge (the benchmark β lever)** (`prompt.py` `build_reasoning_messages` / `build_structuring_messages` now instruct the model to name ONLY the breaching entity + put the action verb in the title; vertical-agnostic wording → energy/supply already pass these checks, no regression expected). **The key finding this session — why the two changes DON'T overlap:** the benchmark grades the RAW `LlmJudgment` (`harness.evaluate_item` bypasses `_compose_action` entirely), so **fix #2 is INVISIBLE to the benchmark β/α score** — validated instead by a dedicated OFFLINE unit test (`test_affected_entities_is_loop_entity_not_llm_overnaming`: an over-naming judgment → asserts the composed envelope names only the single loop entity, Lesson #7 §3 behavioural). The **prompt nudge** is what moves β — to be measured later in a **GATED host-state delta re-run** (NOT run this session). **The entity-key fork, RESOLVED:** `_loop_entity_ref` assumes the faithful ontology-projected event keys `object_type` + `primary_key`, with defensive `.get(..., fallback)` mirroring `recommender._rule_recommend` (degrades to `event_id`/`"unknown"` on a stub event, never raises). The procedure-path event shape is not yet standardised — surfaced for merge-review per a two-way-door call (reversible, offline, no production consumer today since Tier-2 real-data is parked); **Cray confirmed NO Tier-2 event-key contract in mind**, so the defensive getter STANDS (the HEDGE is documented in the `_loop_entity_ref` docstring + the commit body + the PR body). **Verification:** `ruff` + `mypy --strict` (services) green; `pytest` **71** (action_step + prompt + DB procedure + benchmark) + **104** (llm + eval golden-trace + recommender) green → confirms the reactive Pipeline-v0 path + eval golden traces do NOT regress from the shared `prompt.py` edit. PLAN-0020 stays status **Draft** — SD-1 (widen supply-α `valid_handlers` `[hold]`→`[hold, inspect]`; needs Cray re-ratify BEFORE any dataset/grader edit) + SD-2 (8 s-bar review) remain **pending Cray ratification**; Code did NOT touch the grader/dataset. This block = the session-49 #232 reconcile (head_commit `8324cba` — the newest substantive commit per `lint_status`; the #232 merge commit `f46f29c` is lint-excluded). · **Prior context (session 48 — PLAN-0020 (procedure-path tuning) DRAFTED + merged (#230, `d013b66`, `docs(plans):`))** — the PLAN-0019 B-6 ring-fence follow-up tuning PLAN (status **Draft** → Proposed → Accepted on Cray ratification); RING-FENCED from ADR-016, moves no pre-registered bar; G2-approved (number + scope). 4-item scope: latency levers (trim `think=True` call-1 / batching / faster-arch model / 8 s-bar review) · the **Cray-approved deterministic `affected_entities` override (fix #2** — mirrors the `step.handler` override, closes the over-naming envelope/UX leak) · the aquaculture prompt nudge · the **GATED** supply-α `valid_handlers`→`[hold, inspect]`. **SD-1** (supply-α widen — needs Cray re-ratify BEFORE any dataset/grader edit) + **SD-2** (8 s-bar review) pending ratification. NB the G2 PreToolUse classifier briefly fail-closed on an API-Console billing 400 ("credit too low") before drafting — resolved by a $20 console.anthropic.com top-up (separate billing bucket from the Max subscription; see memory). · **PLAN-0019 Part B hardened re-run is COMPLETE (#228, `ec073a4`, `feat(benchmark):`) — the hardened benchmark now DISCRIMINATES as designed, on live `gpt-oss:20b` (MS-S1, Cray-approved host-state run).** Added `--dump-json` to the runner (per-item JSONL: verdicts + per-check details + raw `LlmJudgment`; `harness.py` `ItemResult` carries the raw judgment) so **every score was VERIFIED against real model output, not inferred** (the session-46 "verify, don't infer" lesson); `REPORT.md` gains a "Results — HARDENED run (2026-06-09)" section, the pre-hardening baseline retained for comparison. Touched only `benchmarks/` (REPORT.md, harness.py, run_benchmark.py) — no product code. **Hardened results (198 items / 240 LLM calls / 0 StructuredOutputError): β headline (SD-B1 ≥85%)** aquaculture 60.0% (24/40), energy 97.5% (39/40), supply_chain 100% (40/40) → **overall 85.8% (103/120) — CLEARS ≥85%** (companion warm run 89.2% → honest β ~86–89%); β fell from a pre-hardening 100% — the hard scenarios gave the headline real discriminating power. **α handler-probe (own lane, NOT headline):** aquaculture 77.5%, energy 100%, supply_chain 32.5% → overall 70.0% (84/120). **Sanity 100% (198/198). Latency (B-δ):** p95 22.64 s / mean 15.02 s (240 calls) → OVER the 8 s bar (~2.8×) — same ring-fenced finding (NOT a build failure, NOT a bar move, NOT an ADR-016 reopen). **Verified failure modes (from `--dump-json`):** aquaculture β=60% = two REAL weaknesses (11× `forbidden_primary_keys` — under multi-entity input the model over-names SAFE decoy sibling ponds; 7× `action_keywords` — assessment framing omits the aerate verb); energy & supply_chain show NO over-naming (energy's `forbidden_primary_keys` passed every hard item → real cross-vertical signal). supply_chain α=32.5% = a BENIGN divergence (model picks defensible `inspect` where the dataset pins single `hold`, NEVER the dangerous near-misses `expedite`/`reroute`; β stays 100%) → finding: the α `valid_handlers` for supply_chain is plausibly too narrow (arguably `[hold, inspect]`) — a tuning-PLAN question, NOT a grader change (methodology ratified). **Bottom line: Part B's empirical core is closed end-to-end — the hardened benchmark discriminates, the α probe surfaces a benign handler divergence, sanity 100%, latency the known over-bar B-δ finding; all below-bar reads are logged findings → the follow-up tuning PLAN under the B-6 ring-fence, none moves a bar or reopens ADR-016.** Raw evidence dump = a gitignored working note under `.claude/benchmark-results/`. **NEXT (Cray sequences): (1) latency tuning PLAN** (OFFLINE, via `plan-drafter`; levers trim `think=True` call-1 / batching / faster-arch small model / revisit the 8 s bar; incorporate the supply-α `valid_handlers`-too-narrow observation; MUST NOT reopen ADR-016's primitive shape — ring-fence); **(2) B-3 baselines** (text-to-SQL + RAG, REPORTED not gated — heaviest remaining sub-step, hits MS-S1); **(3) B-5 report finalize + B-6 ring-fence wrap.** · **Prior context (session 47 — PLAN-0019 Part B hardening COMPLETE, PR1 #224 + PR2 #226) — the benchmark now has the real action vocabulary, the β/α grading split, AND the hard multi-entity / near-miss scenarios + precision checks that give the β headline real discriminating power.** **#226 (`e76977d`, `test(benchmark):`) — Part B hardening PR2 of 2 (harder scenarios), the SECOND PR of session 47:** adds the hard multi-entity / distractor / near-miss breach scenarios + the two β-headline precision checks. Cray-ratified design (2026-06-09): the entity-precision check is **`forbidden_primary_keys`**; the hard items **augment** the dataset (the easy items stay as a floor baseline). Sequencing decided ELI-CTO: **build PR2 first, THEN one comprehensive hardened re-run** (not re-run-now-then-again) — higher information yield per host-state window, no misleading intermediate ~100%, respects anti-moving-target. PR2 gives the β headline real discriminating power on the two fields the model genuinely OWNS in the procedure path — entity-ID and action class: (1) **multi-entity decoys (entity-ID):** `Scenario` gains `distractors` (1–3 SAFE sibling readings, mostly in the watch band so they read as borderline), injected into the event as `other_readings`; the model must name the breach AND NOT the decoys → β-headline scoring checks `affected_primary_key` (right entity) + the new `forbidden_primary_keys` (no decoy named); a loader guard asserts every decoy is genuinely non-breaching and that `forbidden_primary_keys` exactly equals the scenario's distractor set. (2) **near-miss action (action-class):** a plausible-but-wrong action the model must avoid recommending — aquaculture `feed` (feeding during an O₂ crash), energy `monitor`/`schedule` (deferring an acute over-temp), supply_chain `expedite`/`reroute` (keeping a possibly-spoiled load moving) → checks `action_keywords` (right verb present) + the new `forbidden_keywords` (decoy verb absent from the proposal TITLE; the body may legitimately rule it out). **Dataset:** +12 HARD breach items per vertical (ids `*-h01..h12`), each a clear breach among safe sibling decoys + a domain near-miss; easy items kept as the floor baseline → **54 → 66 items/vertical; 84 → 120 graded breach items.** `grader.py`/`harness.py`/`schema.py` thread the precision checks + distractor injection; `REPORT.md` documents the hard scenarios + precision checks and keeps the prior filled numbers as the PRE-HARDENING baseline. PR2 touched only `benchmarks/` + `tests/benchmark/` — no product code. Verification: ruff check + ruff format clean; `mypy --strict` (benchmarks) clean; full suite **1347 passed / 2 skipped** (was 1340/2; +7 benchmark tests). **NEXT = a comprehensive hardened re-run on live `gpt-oss:20b` on MS-S1 (192.168.1.133:11434) — a host-state change, ASK Cray before warming/running** — filling the β headline on the hard scenarios + the α handler-selection probe on the real menu (per the agreed PR2→single-re-run ordering); then a latency tuning PLAN, B-3 baselines, B-5/B-6 wrap. · **Prior context (session 47 earlier — #224, `00d7a24`, `feat(benchmark):`) — Part B hardening PR1 of 2 (handlers-first):** the real ontology action-handler vocabulary ((C) product-complete — `verticals/{aquaculture,energy,supply_chain}/handlers.py` register the `RecommendedAction.action_type` names as distinct no-op stubs; each breach step's `step.handler` fixed to the correct `action_type`; autonomy stays `gated`) + the **β-headline / α-probe grading split** driven by the handler-determinism finding (the procedure path OVERRIDES the model's `suggested_handler` guess with the author's deterministic `step.handler`, so grading handler-selection as the headline would measure a procedure-overridden field — β headline = entity + action-class the model owns; α probe = handler-selection on its own reactive-path lane). Full suite **1340 passed / 2 skipped** (+4 net) at that point. **The handler-determinism finding (why β/α):** vero-lite has TWO action paths with different handler semantics — the **reactive** Pipeline-v0 (`recommender._compose_llm_record`) USES the model's `suggested_handler` guess and `execute()` runs it; the **procedure** orchestrator (ADR-016, `action_step._compose_action`) OVERRIDES the guess with the author's deterministic, allowlist-bounded `step.handler`. The benchmark grades the raw `LlmJudgment`, but PLAN-0019 validates the **procedure** path, which discards the handler guess — so grading handler-selection as the *headline* would measure a field the procedure product overrides (and brush ADR-016). Resolution, Cray-ratified 2026-06-09: **β headline** = the fields the model owns in the procedure path = entity (`affected_primary_key`) + action class (`action_keywords`); **α probe** = handler-selection (`suggested_handler` vs the correct ontology `action_type`), reported on its OWN lane as a reactive-path / future-autonomy signal, NOT folded into the headline. `payload_contains` stays advisory. **Product ((C) product-complete):** `verticals/{aquaculture,energy,supply_chain}/handlers.py` now register the ontology `RecommendedAction.action_type` vocabulary as distinctly-named **no-op stubs** alongside `echo` (aquaculture `start_emergency_aerator`/`dispatch_technician`/`increase_water_exchange`/`escalate`; energy `restart`/`isolate`/`dispatch_technician`/`escalate`; supply_chain `reroute`/`expedite`/`hold`/`inspect`/`escalate`) — so the LLM `suggested_handler` enum is now a real 4–5-option menu (real I/O still lands with the design partner). Each procedure's breach action step now fixes `step.handler` to the correct `action_type` (aquaculture `aerate`→`start_emergency_aerator`, energy `restart_breaches`→`restart`, supply_chain `hold_breaches`→`hold`), **autonomy stays `gated`** (the human go/no-go is unchanged), and each agent's `allowed.action_handlers` allowlist was updated; aquaculture `summary` stays `echo` (a no-op artifact). So the OCT product now proposes/executes real-named gated actions instead of `echo`. **Benchmark:** `grader.py` `FieldCheck` gained a `probe` lane + `GradeResult.probe_passed`; `harness.py` `Summary`/`ItemResult` thread the α metric; `run_benchmark.py` prints it. Dataset `valid_handlers [echo]→[correct action_type]` across all 84 breach items. The dataset-consistency guard now requires ≥1 β-headline scoring field; a new offline test asserts every α-probe handler is registered for its vertical. `REPORT.md` documents the finding + the β/α lanes and **relabels the prior filled numbers as the PRE-HARDENING baseline** (echo-only, valid_handler-in-headline) — a hardened re-run is pending. `pyproject.toml` adds `allowed-confusables = ["α","β"]` for the project's Greek step/metric notation. `tests/services/db/test_procedure_headline.py` binds its gate spy to `start_emergency_aerator` (the gated step's new handler). **Verification:** ruff check + ruff format clean; `mypy --strict` (services/verticals/benchmarks) clean; full suite **1340 passed / 2 skipped** (was 1336/2; +4 net benchmark tests). · **Prior context (session 46 — B-δ COMPLETE, #222, `957a657`, `test(benchmark):`) — per-call latency instrumentation + the SD-B1 p95 measurement for the pin, plus the B-4/G-3 four-model selection sweep and a runner-robustness fix.** **Headline outcome: the latency bar is MISSED and is a logged finding → tuning PLAN; the ADR-0001 pin `gpt-oss:20b` HOLDS.** **B-δ Part 1 — latency instrumentation (`a6d3f62`):** added a `TimingChatClient` decorator (times each `chat()` call into a shared `LatencyRecorder`) + `percentile` (nearest-rank) + `summarize_latency` to `benchmarks/procedure_baseline/harness.py`; `run_benchmark` wraps its client and reports p95 vs a new `--latency-threshold` (default 8.0). Live `gpt-oss:20b` warm-first run (full 162 items / 168 calls): accuracy **97.6% (82/84)** this run — **run-to-run non-deterministic** (the #220 run was 100%; honest read **~98–100%**); the two misses **both inclusive-boundary breaches** (DO 4.0, 90.0 °C) → boundary cases are the failure mode; **latency mean 13.0 s / p50 12.1 s / p95 19.2 s / max 22.5 s → OVER the SD-B1 ≤ 8 s bar (~2.4×)** (the `think=True` call-1 reasoning dominates per-call time). **B-δ Part 2 — G-3 four-model sweep + runner robustness (`2476ff7`, `957a657`):** a robustness gap surfaced — a slow model exceeding the 120 s per-call timeout raised `OllamaError` and crashed the run → `evaluate_item` now records a transport error as an error-tagged failed proposal and continues (+1 test). Sweep (serialized so MS-S1 stayed quiesced; MS-S1 Ollama updated to **0.30.6**; pin ran 84 breach items, candidates a 9-item breach subset, each re-warmed): **`gpt-oss:20b`** (20 B, pin) ~98–100% acc, **13.0 s mean / 19.2 s p95** — best on both axes; **`gemma4:12b`** (12 B) **100% (9/9)**, reliable JSON, **45.9 s mean / 81.1 s p95** (clean, un-clipped); **`qwen3.6:35b`** (35 B) ~87.5% (7/8; 1 timeout), 46.9 s mean / 120 s p95 (timeout-clipped) — **this CORRECTS the prior "qwen3.x = NOT_JSON" note: qwen3.6 DOES emit valid structured JSON on this build**; **`gemma4:26b`** (26 B) not measurable (8/9 errored — 7 timeouts + 1 malformed JSON), 51.7 s mean / 120 s p95. **Key finding — smaller ≠ faster:** `gemma4:12b` (smaller than the pin) is ~3.5× SLOWER; per-call latency is dominated by generated-token count + architecture/quant, not param count. **G-3 conclusion (closes the evidence gap):** across 12 B–35 B the pin `gpt-oss:20b` is best on BOTH axes and ~3.5× faster than every alternative; going smaller did not help, so the latency miss is **not** a param-count problem and is **not** solved by any available model — **the pin HOLDS**; levers (trim the `think` pass / batching / a faster-arch small model not yet on MS-S1 / revisit the 8 s bar) belong to the follow-up tuning PLAN. All of B-δ **REPORTS — it does not gate** (B-6 ring-fence); the latency miss is a logged finding, not a build failure, and not a reason to move the bar. Results live in `benchmarks/procedure_baseline/REPORT.md` (headline + sanity + latency + the 4-model G-3 table, all caveated). Verification: ruff clean; `mypy --strict` clean; offline benchmark suite **35 passed**; full suite **1336 passed / 2 skipped**. · **Prior context (session 46 earlier — B-β COMPLETE, #217 scaffold + #219 fill + #220 calibration/run):** the live HEADLINE ran — **LLM action-proposal correctness 100% (84/84)** clearing the SD-B1 ≥85% bar (caveated as a FLOOR — `echo`-only handler + well-posed single-entity breaches), deterministic disposition sanity 100% (162/162), 0 `StructuredOutputError`; `REPORT.md` records it WITH the load-bearing caveat (REPORTS, does not gate). #219 grew the seed datasets to **54 items/vertical (162 total)** = the Cray-ratified SD-B2 size; #217 was the OFFLINE scaffold-first thin slice that proved the SD-B1 graded-unit-A design before mass authoring. · **Original session-46 B-β narrative continues below.** **PLAN-0019 Step B-β was COMPLETE — the dataset was filled to SD-B2 size (#219, `73170d7`, `test(benchmark):`) then the harness was calibrated and the LIVE HEADLINE run was executed (#220, `ae7221a`, `test(benchmark):`); LLM action-proposal correctness = 100% (84/84), clearing the SD-B1 ≥85% bar (with a load-bearing caveat).** **#219 (FILL):** grew the three seed datasets from ~7 to **54 items/vertical (162 total)** = the Cray-ratified SD-B2 size (~50–60/vertical), breach-weighted **28 breach / 13 watch / 13 ok per vertical (84 graded breach items)** with dense inclusive-boundary clusters (aquaculture DO 3.9/3.95/3.99/4.0; energy 90.0/90.1/90.3/90.5; supply_chain 8.0/8.1/8.2/8.3); the `tests/benchmark` self-consistency test asserts every item's declared disposition matches `classify_disposition`. Data + REPORT denominators only; offline. **#220 (CALIBRATION + LIVE RUN):** a pre-run smoke against live `gpt-oss:20b` on MS-S1 surfaced that the **harness, not the model, was mis-measuring** — four **measurement-correctness** fixes were **Cray-ratified BEFORE the scored run** (anti moving-target — none moves the ≥85% bar or tunes-to-pass): (1) `scenario_to_event` injects the domain `parameter` (`dissolved_oxygen`/`temperature`) so the model knows what is measured (faithful to a real ontology event) + a required `reading_parameter` field added to the `Dataset` schema + the three dataset files; (2) `payload_contains` → **advisory** (recorded, not a headline gate) since the live model's `handler_payload` keys are free-form (`event_id`/`action`/`recommendation`, never the guessed `pond_id`) — `FieldCheck` gained an `advisory` flag; (3) `action_keywords` broadened to per-vertical action lemmas (aquaculture `[aerat, oxygenat]`, energy `[restart, reset, reboot]`, supply_chain `[hold, inspect, quarantine, divert]`); (4) `action_keywords` searches `rationale` too (title/description/rationale) — the model often places its proposed action in `rationale` (the diagnostic showed every aquaculture "fail" already said "Aeration" there). Smoke trajectory **44% → 61% → 100%**. **Live run (all 162 items; 84 breach → 168 LLM calls; `gpt-oss:20b` on MS-S1, Cray-warmed + Cray-approved):** **Headline (LLM action-proposal correctness) = 100.0% (84/84)** — aquaculture 28/28, energy 28/28, supply_chain 28/28, clears SD-B1 ≥85%; **deterministic disposition sanity = 100.0% (162/162)**; **0 `StructuredOutputError`**; inclusive-boundary breaches (DO 4.0 / 90.0 °C / 8.0 °C) all pass. `benchmarks/procedure_baseline/REPORT.md` records the result WITH a **load-bearing caveat**: the 100% is easy-by-construction (`echo`-only handler ⇒ `valid_handler` trivially satisfied; well-posed single-entity breaches), so it is a **FLOOR** — "the well-posed path works end-to-end," NOT "the model is infallible." Per the B-6 ring-fence this REPORTS, it does not gate. **Latency (SD-B1 ≤8 s p95/call) was NOT measured** — the runner isn't instrumented for per-call timing; that's B-δ. Verified across both PRs: ruff clean; `mypy --strict benchmarks tests/benchmark` clean; offline benchmark suite **30 passed**; full suite **1331 passed / 2 skipped**. · **Prior context (session 46 earlier — B-β SCAFFOLD, #217, `367503c`, `test(benchmark):`):** the FIRST Part B build PR — a scaffold-first OFFLINE thin slice of the procedure-baseline benchmark (`benchmarks/procedure_baseline/` schema/loader/grader/harness/run-skeleton + ~7 seed items/vertical + REPORT placeholder + 28 offline tests under `tests/benchmark/`, additive only) proved the Cray-ratified SD-B1 graded-unit-A design end-to-end before the mass authoring that #219/#220 then did. `grader.py` reuses the engine's own `crosses_threshold` as the single breach-decision source of truth. Verified: ruff + `mypy --strict` clean; full suite **1329 passed / 2 skipped** (+28). · **Prior context (session 45):** **PLAN-0019 Part B pre-registration RATIFIED *in the binding plan* (#214, `9d98f03`, `docs(plans):`) + the L1 loop-detect guard fixed (#215, `d96e69f`, `fix(hooks):`) → the HARD GATE was CLEARED and Part B went GO.** **Step 0 (#214):** the Cray-ratified Part B surfaced decisions were written into PLAN-0019 §8 (they were ratified in conversation but not yet in the plan — closing the anti-moving-target gap before any dataset authoring): **SD-B1 → ✅ RATIFIED** (numbers unchanged: procedure-recommendation accuracy ≥ 85%, p95 per-LLM-call latency ≤ 8 s on `gpt-oss:20b`; #211 had added the operational definitions but left the §8 header in "Recommendation" framing — now flipped); **SD-B2 → ✅ RATIFIED ~50–60 questions/vertical (~150–180 total)** (Cray chose the larger set over the draft's ~30/vertical for external-grade statistical defensibility — ~30's ±~13pp CI too wide); **SD-B3 → ✅ RATIFIED: DEFER** (the G-2 build-cost-via-`new-vertical`-generator measure stays out of Part B — generator not built, tangential to engine quality). Also fixed a stale `SD-B3`→`SD-B2` cross-ref in AC B-1 / Step B-β / the §8 header note and flipped frontmatter `status: Draft` → `In execution`. Authored by the `plan-drafter` subagent (6 of 8 edits) + Code (the 2 remaining SD-B2/SD-B3 flips, after an L1 loop-detect turn-boundary reset). **The L1 fix (#215):** `fix(hooks):` L1 path-class threshold (docs 15 / code 6) + subagent-completion reset — fixes the L1 loop-detect guard that was false-firing on legitimate documentation authoring (it bit THIS session: a `plan-drafter` subagent's 6 edits to PLAN-0019 exhausted the flat-6 turn budget so the main agent could not add even one more edit). `_loop_counter.l1_threshold_for` now gives **6 for code paths (unchanged), 15 for prose/doc paths** (`*.md`/`docs/` — finite, so a stuck doc loop still trips); `posttooluse_progress_observer._handle_agent_completion` resets L1 for files touched this turn when an `Agent`/`Task` tool completes (so a subagent's edits don't pre-spend the main agent's budget); the deny message + `.claude/autonomy-triggers.md` row L1 were updated and **Lesson #0021** records it (sequel to Lesson #0012). Cray-approved per-diff self-modification (the auto-mode classifier correctly denied the first attempt as self-modification under a mere direction pick); Cray chose "A now, B later" — distinctness-based counting (option B) deferred as a follow-up. Verified: ruff + mypy --strict (.claude/hooks) clean; full suite **1301 passed / 2 skipped** (+25 tests). · Prior context (session 44): **SD-B1 RATIFIED (#211, `812eae7`, `docs(plans):`) → Part B is UNBLOCKED.** Before any dataset authoring, Code's pre-ratification review pinned SD-B1's operational definitions: `accuracy` = **procedure-recommendation correctness** on the LLM `action`-reasoning path (`generate_judgment`) graded vs the synthetic key — NOT the deterministic `evaluate` threshold rule (`crosses_threshold`, ~100% by construction); `latency` = p95 **per-LLM-call** (warm, quiesced MS-S1), not per set-valued step. Numbers unchanged (≥ 85% accuracy / ≤ 8 s p95 / `gpt-oss:20b`); AC B-2 (numbers ratified WITH definitions) + B-4 (per-LLM-call) aligned; B-3 left as-is. **SD-B2 (dataset ~50–60/vertical ≈ 150–180 total, for an external-grade CI over ~30's ±~13pp) + SD-B3 (build-cost DEFER) also RATIFIED (Cray, this session) — the full Part B pre-registration is locked.** A clean checkpoint was cut here for session 45 (Part B = a sizeable dataset + harness + live-LLM chunk); next = formalize SD-B2/B3 into PLAN §8 then B-β author the dataset (handoff `.claude/handoffs/session-45/2026-06-08-1800-code-session45-part-b-kickoff.md`). Also this session: the owed `## Current Focus` narrative refresh landed (#210, `9f19949`). Earlier this session: **PLAN-0019 Part A COMPLETE: A-ζ (three example procedures) + A-η (headline integration test) landed via #208 (`5b2c189`, `feat(engine):`)** — Part A (A-1…A-12) is DONE, reaching the **HARD GATE** before Part B. **A-ζ (AC A-10):** three `verticals/<name>/procedures.yaml` specs (config only; no new executors) — aquaculture **"Morning Pond Health Round"** headline (`read_do`→`judge`→`aerate` gated over the breach subset via `input: {from: judge, where: {verdict: breach}}`→`visual` human_task over the watch subset→`summary` auto over the whole verdict set), energy "Substation Health Sweep" + supply_chain "Cold-Chain Excursion Sweep" (each a `query→evaluate→gated action` path); handler = `echo` everywhere (the only registered handler; `start_emergency_aerator`/`restart`/`hold` noted as the intended `action_type`, deferred). A new pure-Python load test (`test_example_procedures.py`, 10 cases) asserts all three load + validate (cross-refs resolve, named-input references linear, handlers allowlisted). **A-η (AC A-11):** `tests/services/db/test_procedure_headline.py` drives the REAL shipped aquaculture headline manually — FAKE query/evaluate/human_task executors + the **REAL `ActionStepExecutor`** (mock `ChatClient`, NO live LLM) — and asserts behaviourally (Lesson #7 §3): the breach/watch named-input fan-out, the durable suspend→`resolve_gated_step(approve)`→resume lifecycle (`echo` fires once per breach pond; run `completed`), the auto summary over the whole set, the telemetry seam on every StepResult, and **reject = continue + record** (a rejected breach never fires its handler, the run still completes). Purely additive; no engine code touched. Verified (AC A-12): ruff + ruff-format + mypy --strict clean (47 files); full suite **1276 passed / 2 skipped** (+12; DB tests run live). **→ HARD GATE: Part B does not start until SD-B1 thresholds are Cray-ratified.** Prior in-session context (session 43): **A-ζ-prep named-input references + field-equality fan-out filter (#206, `42739b8`, `feat(engine):`)** — the orchestrator now keeps a NAMED-OUTPUT BAG (every step's output keyed by step_id); `Step.input` became structured `StepInput {from, where}` (`from` = a named prior step, default = the immediately prior one — backward-compatible; `where` = field-equality filter `entity[field]==value`). Enables the headline fan-out: one `evaluate` set → breach/watch/ok subsets that distinct later steps consume (`input: {from: judge, where: {verdict: breach}}`). `validate_runnable` pre-checks references are linear/backward; `resume_run` rebuilds the bag from the DB so a resumed step can reference any earlier named step across a restart (old `_prior_output` hack removed). **Cray's design call (over pass-through).** Verified: ruff + mypy --strict clean; full suite **1264 passed / 2 skipped** (+7); the existing 41 procedure tests unchanged. Prior in-session context: **A-7 action-step adapter landed via #204 (`56ab5f3`, `feat(engine):`)** — the real `action` StepExecutor (`services/engine/procedures/action_step.py`): per breach entity it builds an ADR-007 `RecommendedAction` (envelope UNCHANGED; `suggested_handler = step.handler`, allowlist-bounded) with reasoning via the mockable `generate_judgment` (NO live LLM in tests), and routes through the shipped `approve()`→`execute()` gate VERBATIM. **Option 2 (external gate):** `gated` actions only PROPOSE → suspend; `resolve_gated_step` (the external driver = the shipped `recommender` gate) applies the human's approve/reject, rewrites the step `output_set`, persists; `resume_run` is UNCHANGED and just threads the resolved output forward. **Reject = continue + record** (handler never fires, rejection in the trace, run reaches `completed` not `failed`); `auto` actions approve+execute inline. Verified: ruff + mypy --strict clean; full suite **1257 passed / 2 skipped** (+9); no existing source touched. **A-ε (A-7 + A-8) DONE.** Prior in-session context: **A-8 goal injection landed via #202 (`e857c14`, `feat(engine):`)** — `Procedure.goal` (a trusted authored-config directive, ADR-016 D5) is injected into the LLM system prompt for `evaluate` + action reasoning across BOTH Pattern-B calls and threaded onto `RunContext.goal` (run + resume); the `goal=None` default keeps the reactive Pipeline-v0 path byte-identical, and the goal rides in the trusted system instruction (never the untrusted block). Verified: ruff + mypy --strict clean; full suite **1248 passed / 2 skipped** (+8 tests). **A-ε design forks DECIDED this session (land in the A-7 PR next):** the gated-action `approve()`→`execute()` runs via the **external** gate (**Option 2** — `resume_run` reads the human decision + continues, staying a pure control-plane continuation), and a human **REJECT = continue + record** (grounded in a Palantir staged-Action research pass — Palantir has no run primitive, so a reject is a local "not applied" disposition, NOT a failure; the heavyweight branching/Approvals reject-blocks-merge workflow is the deferred governance layer, out of Phase-1 scope). Prior in-session context: **Session 42 — PLAN-0019 Part A Step A-δ landed via #199 (`4e90b18`): durable suspend/resume** (ADR-016 D4, AC A-5). `services/engine/procedures/persistence.py` adds `persist_run` / `load_run` (Postgres) + `resume_run`, which resumes a `waiting_human` run reconstructing state purely from the DB so a fresh process can resume a run another started; the orchestrator step-loop was refactored into a shared `execute_steps` (DB-free engine; persistence is the seam). The completed prefix is never re-executed (verified by executor call-count). A **re-run fix** (the 2nd #199 commit) corrects resume of an `escalate_to_human` failure: instead of marking the FAILED step complete + threading empty input forward, resume now **re-runs** the failed step from its original input (overwriting the stale failed record) so a human can fix the cause and retry. Verified: ruff + mypy --strict clean; full suite **1240 passed / 2 skipped** (DB tests run live). Prior in-session context: **PLAN-0019 Part A build CONTINUES — the orchestrator control plane landed (#197, head `9e9909e`, `feat(engine):` + `refactor(engine):`): Step A-γ.** `run_procedure()` (`services/engine/procedures/orchestrator.py`) runs a procedure's steps linearly, threads each step's output **set** into the next (A-3 set-valued, no loop), enforces the D3 autonomy model + agent blast-radius bound (`validate_runnable`: manual-only trigger per L-1; step-kind allowlist; action autonomy ≤ `autonomy_ceiling`; action `handler` ∈ `allowed.action_handlers`), suspends at `waiting_human` on a `gated` action or `human_task`, and does D4 fail-and-divert (`failed`, or `waiting_human` when `on_failure = escalate_to_human`). Every `StepResult` carries the telemetry seam (A-9). `spec.py` gained `Step.handler` (action-only, allowlist-checked). A robustness-hardening follow-up (`9e9909e`) makes the orchestrator **fail-safe by construction** — a misbehaving executor (raising or returning a non-`StepOutcome`) is **diverted, never crashes** the run loop; the failed-step trace retains the exception type; the gated-action write-before-approval safety contract (enforced by the ADR-007 approve→execute gate, not the orchestrator) is documented. Still **additive + in-memory** — no persistence yet. Delivers **A-3** (linear set-valued orchestrator), **A-4** (autonomy + ceiling + handler allowlist), **A-6** (fail-and-divert), the **A-9** telemetry recording in the run path, **A-12** (invariants green; ruff + mypy --strict clean; full suite **1235 passed / 2 skipped**). Drafted + verified + committed by Code. Prior in-session context: **PLAN-0019 Part A build STARTED — the first build PR landed (#195, `6ef3a57`, `feat(engine):`): the Procedure spec layer (Step A-α) + run-record persistence (Step A-β).** The `Procedure/Step/Agent` Pydantic SPEC models + a `ruamel.yaml` loader (`services/engine/procedures/spec.py`), and the durable `pipeline_runs`/`step_results` ORM + Alembic migration `0002` (`services/engine/procedures/runs.py`). Additive only — NO orchestrator yet; the ADR-007 `RecommendedAction` envelope + the ADR-008 ontology are untouched. The D3 autonomy invariant is enforced in the spec (autonomy on `action` only, default `gated`; non-action steps that set autonomy are rejected); the per-step telemetry seam (`duration_ms` BIGINT + `reasoning_trace` + `audit` JSONB) that Part B consumes is in place (AC A-9); `trigger ∈ {manual, schedule}` both load but only `manual` is runnable in Phase 1 (L-1). Delivers **A-1** (spec loaders), **A-2** (run records + migration), **A-9** (telemetry columns), **A-12** (ruff + mypy --strict + full suite green: 1221 passed / 2 skipped; DB tests run live — migration applies + ORM round-trips). The energy DDL↔ORM parity guard (`tests/services/db/test_schema_parity.py`) was scoped to the energy ontology module since the new tables share `Base.metadata` but are cross-vertical engine infra. Drafted+verified+committed by Code. Prior in-session context: **PLAN-0019 (Core Procedure baseline, Phase 1) earmarked, drafted, reviewed, and committed via #193 (`69c1ddf`, `docs(plans):`).** PLAN-0019 implements **ADR-0016 Phase 1** as a **MERGE-with-guardrails**: one PLAN, two internal Parts, a HARD GATE between them. **Part A — Engine** (deterministic functional acceptance): the `Procedure/Step/PipelineRun/Agent` runtime + a linear, set-valued, sequential orchestrator over `{query, evaluate, action, human_task}`; default-`gated` actions + autonomy-ceiling + handler allowlist; durable suspend→resume; fail-and-divert; **reuses the shipped ADR-007 `RecommendedAction` envelope + approve→execute gate verbatim**; a mandatory per-step telemetry seam feeds Part B; lands via `feat/*` PRs. → **HARD GATE** (Part B may not start until Part A is green) → **Part B — Benchmark** (empirical acceptance): pre-registered absolute thresholds; the text-to-SQL + RAG comparison is **REPORTED, not gated**; below-threshold = a logged finding → a follow-up tuning PLAN that **must not reopen ADR-0016's primitive shape** (ring-fence); closes evidence-gap **G-3** (per-procedure local-LLM model selection); lands via its own `test/*` PR. This folds the former "Thread 2" into PLAN-0019's acceptance under guardrails. **LOCKED scope:** **L-1** `manual` trigger only (orchestrator trigger-agnostic; `schedule` deferred to a PLAN-0010 reuse); **L-2** three OCT example procedures (aquaculture "Morning Pond Health Round" headline + energy + supply_chain; `vet_clinic` excluded, parked ADR-005); **L-3** Postgres persistence via Alembic. **Architectural SDs ratified this session:** **SD-A1** (JSONB `pipeline_runs`/`step_results` schema) + **SD-A2** (engine home `services/engine/procedures/`). **B-side SDs stay OPEN** (resolved at their execution step): **SD-B1** pre-registered thresholds (MUST be Cray-ratified before any Part B run — anti moving-target), **SD-B2** synthetic-dataset size, **SD-B3** the optional G-2 build-cost measure. **Drafted by the `plan-drafter` subagent, reviewed by Code (reuse symbols verified against live `services/engine/{actions,recommender,registry}.py`), G2-approved + ratified by Cray, committed via #193.** Prior context: Session 41 — **strategy/design session that worked Thread 1 (of the two session-41 kickoff strategy threads) all the way to a ratified ADR.** **ADR-0016 — Governed Procedure Engine — ratified Accepted + merged (#190, `949eaea`, `docs(adr):`).** It expands the OCT action layer from reactive-only `anomaly→action` to **`anomaly AND normally→action`** via a governed, human-gated **Procedure engine** — a reusable cross-vertical capability (capability/decision document only; implementation is PLAN-0019). Arc: started from the session-40 dual-track impl-approach research (the action layer is vero-lite's differentiator); worked Cray's vision (stakeholder narrative → ontology → **pipeline** of goal/decision/tool-call/trigger/terminal/human-gate, OCT as the Command/Control/Monitor center, referencing Palantir Foundry pipeline docs, scoped DOWN to a tangible/measurable/extensible baseline) through **path 2 (shape-sharpening)** then **path 3 (stress-test vs the real aquaculture ontology)**, iterating shape v0→**v3.3**. Two design seams caught + resolved: human_gate collapsed into the `gated` autonomy property; human-task relocated from autonomy class → step `kind`. Grounded twice against Palantir — a quick WebSearch/WebFetch pass, then a **`deep-research` workflow** (5 concerns, **25 claims verified 3-0 / 0 killed**, all primary palantir.com/docs) → saved private/gitignored at `docs/research/private/2026-06-07-palantir-5-concerns-pipeline-design.md`: agent-as-actor (→ first-class `Agent`), Automate fail-and-divert failure semantics (NO run-status enum → we add one), per-WRITE approval (autonomy on `action` only), goal = runtime LLM directive, engine-vs-config = Marketplace bundles (validates the `services/` engine + `verticals/<name>/` config split + Rule-of-Three). The ADR records the **`Procedure / Step / PipelineRun / Agent`** primitive (kind = query/evaluate/action/human_task; autonomy auto/gated on `action` only, default gated; set-valued linear steps; durable/resumable runs; goal=LLM directive; local-LLM bindable per Agent, default `gpt-oss:20b`), purely **ADDITIVE** (does NOT touch the ADR-007 `RecommendedAction` envelope nor the ADR-008 six-`object_types` ontology), positioned at the safe end of the agentic spectrum. Drafted by the `plan-drafter` subagent, reviewed by Code, **G2-approved + ratified by Cray**. Prior context (session 40, carried): first item off the session-40 backlog menu shipped: the standing **ADR-0015 §7 citation errata** (J-class, non-blocking) landed as **#175 (`45012de`, `docs(adr):`)** — ADR-0015 Decision **D5**'s human-review citation `research §7 risk` → `fact-pack §6` (research §7 is the Sources URL list, not a risk; the SOTA-consensus human-review substance lives in the session-35 feasibility fact-pack §6, which ADR-0015's own Consequences already cites correctly). One line changed; no decision/substance touched. **G1 always-pause (mutate an Accepted ADR) was explicitly Cray-approved for the exact diff before the edit.** Context (carried from session 39, unchanged): **PLAN-0017 — the OCT Tier-1 Mirror-demo / live co-creation intake FACE — stays Done** (in `done/`), so the Mirror-demo capability is complete. Also session 40: **run-oct-demo §5b reviewed + fixed** (#177, `2219da1`, `docs(runbook):`) — a code-grounded review of the live co-creation walkthrough surfaced one real bug + three completeness notes (**F1** the §5b worked example booted vertical #4 on port 8099, the §3a aquaculture showcase's own port → live-demo collision; moved to free port 8100; **F2** a §8 409-clobber-guard troubleshooting entry; **F3** the `/intake/*` routes added to References; **F4** a header PLAN-0016/0017 lineage note). The live-demo runbook is now collision-free. Plus a UI polish (#179, `7c9c1f2`, `feat(ui)`): surfaced from a live aquaculture-demo log showing repeated `GET /favicon.ico 404` (the harmless browser default-icon probe — StaticFiles at `/` had no favicon), added an OCT favicon (`static/assets/favicon.svg`, reusing the blue operational-grid + green-status-dot shell identity) + a `<link rel="icon">` so modern browsers use it and stop probing `/favicon.ico`; the partner-facing tab now shows a real OCT mark. Then a demo-prep docs add (#181, `3653d13`, `docs(runbook):`): bilingual (ไทย/EN) per-vertical demo narratives for all three pre-built verticals (§6a — scene→A→B→C→D→number, grounded in the verified on-screen values) + a new **§6b Screen E "Build a Vertical"** section (the missing 4th-vertical / live-co-creation narrative — the 8-step show-sequence within the #3 demo, the golden pivot question, the human-gate-as-feature framing, boot #4 on 8100, fallbacks, closing line); §6 also gained the missing Screen E bullet. Finally a header UI fix (#183, `cc7f3d3`, `fix(ui)`): the demo shell's top bar (brand + A–E nav + chips + MS-S1 + Refresh) overflowed ~1280px screens — the MS-S1 residency control (which gates NL query + Build-a-Vertical) clipped off the right edge (surfaced from Cray's live aquaculture demo; full header ~1487px wide). Re-proportioned by importance: pinned the status + nav zones so they never clip, shed least-important-first across breakpoints (≥1500 all shown; 1201–1499 incl. ~1280 drop the context chips + Refresh→icon, keeping full A–E labels + the MS-S1 control; ≤1200 tabs collapse to A–E keys). Live-verified via Preview at 1600/1280/1200 (0 overflow, MS-S1 fully visible). Follow-up **#185** (`3aac38b`, `fix(ui)`) keeps the **VERTICAL identity chip visible at every width** (per Cray: multiple verticals run side-by-side on different ports, so each window must always show which one it is): only the redundant NS/version chips drop on narrow screens, and the room is reclaimed by collapsing the *inactive* tab labels to A–E keys (the active screen keeps its label) — the MS-S1 status still never clips (verified 1600/1440/1280).
current_actor: code
blocked_on: Nothing gates shipped work. **ADR-0018 "Axis-B Verification Loop" is RATIFIED Accepted (#242, head_commit `1be60f7`, `docs(adr):`) — the Axis-B track moves from deferred → Accepted → entering build.** This reconcile is the second commit on the open PR #242 branch (Code commits it as `docs(status):`); after #242 merges, NEXT = T2 (dispatch the Axis-B build PLAN to Cowork per ADR-009 D1), then T3 (autonomy-triggers V-row), then build. Non-blocking deferrals: OQ-8 (plugin packaging, MS-S1 local evaluator, blocking-mode promotion, PR-merge gating, auto-declared goals) + VX-1..3 = verify-at-execution. · **Prior: main clean @ `129c3a5` (merge #240, the OQ-B reconcile); ADR-0017 arc FULLY COMPLETE (OQ-B resolved, #239).** **CLAUDE.md slimmed: git + Code-ops procedures extracted to on-demand project skills (#234, `471bcb5`, `docs(constitution):`) — `.claude/skills/{git-workflow,code-operational-policy}` added, all binding rules retained in CLAUDE.md.** **1 open PR (#242, this branch)** (untracked `.claude/launch.json` = local Preview config + `.claude/benchmark-results/` = the gitignored raw-evidence dump from the hardened re-run — both local working notes, keep). · **Prior context (session 49 earlier): main clean @ `f46f29c` (merge #232); PLAN-0020 Phase 1 (OFFLINE) COMPLETE + merged (#232, `8324cba`, `feat(engine):`) — fix #2 deterministic `affected_entities` override + the aqua prompt nudge; PLAN-0020 status Draft, SD-1/SD-2 pending Cray ratification.** · **Prior context (session 48): main clean @ `c79248b` (merge #230); PLAN-0020 (procedure-path tuning) DRAFTED + merged (#230, `d013b66`, status Draft — awaiting Cray ratification of SD-1/SD-2).** **ADR-0016 Accepted**; **PLAN-0019 Part A COMPLETE + Part B pre-registration RATIFIED *in the binding plan* (#214); Steps B-β + B-δ COMPLETE (session 46); Part B hardening COMPLETE (PR1 #224 + PR2 #226, session 47); Part B hardened re-run COMPLETE (#228, session 48) — the benchmark DISCRIMINATES (β ~86–89% CLEARS ≥85%, α probe surfaces a benign supply_chain handler divergence, sanity 100%, latency the known over-bar B-δ finding).** **#228 (`ec073a4`, `feat(benchmark):`) — the hardened re-run + `--dump-json` verify tooling, session 48:** every score VERIFIED against real model output (per-item JSONL of verdicts + per-check details + raw `LlmJudgment`); hardened results 198 items / 240 LLM calls / 0 StructuredOutputError; β overall 85.8% (103/120; warm companion 89.2%), α 70.0% (84/120), sanity 100% (198/198), latency p95 22.64 s (~2.8× over the 8 s bar = a logged B-δ finding, ring-fenced). Touched only `benchmarks/` — no product code. Suite unchanged (results-only edits). **NEXT = the latency tuning PLAN (OFFLINE, via `plan-drafter`; MUST NOT reopen ADR-016's shape), then B-3 baselines (text-to-SQL + RAG, REPORTED not gated — heaviest remaining; hits MS-S1), then B-5/B-6 wrap.** Sequencing = Cray's call. **#226 (`e76977d`, `test(benchmark):`) — Part B hardening PR2 of 2 (harder scenarios), the SECOND PR of session 47:** adds the hard multi-entity / distractor / near-miss breach scenarios + the two β-headline precision checks that give the headline real discriminating power. Cray-ratified design (2026-06-09): entity-precision check = **`forbidden_primary_keys`**; the hard items **augment** the dataset (easy items stay as a floor baseline). `Scenario` gains `distractors` (1–3 SAFE sibling readings → injected as `other_readings`) so the model must name the breach AND NOT the decoys (β checks `affected_primary_key` + the new `forbidden_primary_keys`); a domain near-miss action (aquaculture `feed`, energy `monitor`/`schedule`, supply_chain `expedite`/`reroute`) the model must avoid (β checks `action_keywords` + the new `forbidden_keywords` on the proposal TITLE). **+12 HARD breach items/vertical (`*-h01..h12`) → 54 → 66 items/vertical; 84 → 120 graded breach items.** `grader.py`/`harness.py`/`schema.py` thread the precision checks + distractor injection; `REPORT.md` keeps the prior filled numbers as the PRE-HARDENING baseline. PR2 touched only `benchmarks/` + `tests/benchmark/` — no product code. Verification: ruff + `mypy --strict` (benchmarks) clean; full suite **1347 passed / 2 skipped** (was 1340/2; +7). **NEXT = a comprehensive hardened re-run on live `gpt-oss:20b` on MS-S1 (192.168.1.133:11434) — a host-state change, ASK Cray before warming/running** (fills the β headline on the hard scenarios + the α handler-selection probe on the real menu, per the agreed PR2→single-re-run ordering); then a latency tuning PLAN (must NOT reopen ADR-016's primitive shape, ring-fence); B-3 baselines (text-to-SQL + RAG, REPORTED not gated — heaviest remaining sub-step); then B-5 report finalize + B-6 ring-fence wrap. · **Prior context (session 47 earlier — #224, `00d7a24`, `feat(benchmark):`) — Part B hardening PR1 of 2 (handlers-first):** the real ontology action-handler vocabulary ((C) product-complete — distinct no-op `action_type` stubs; each breach step's `step.handler` fixed to the correct `action_type`; autonomy stays `gated`) + the **β-headline / α-probe grading split** driven by the handler-determinism finding (the procedure path OVERRIDES the model's `suggested_handler` with the author's deterministic `step.handler` → β headline = entity + action-class the model owns; α probe = handler-selection on its own reactive-path lane). Full suite **1340 passed / 2 skipped** (+4 net) at that point. · **Prior context (session 46 — B-β + B-δ):** B-β (#217 scaffold + #219 fill + #220 calibration/run) live headline **LLM action-proposal correctness ~98–100%** (caveated as a FLOOR — echo-only handler + well-posed single-entity breaches), disposition sanity 100% (162/162). B-δ (#222, `957a657`) per-call latency **p95 19.2 s / mean 13.0 s OVER the SD-B1 ≤8 s bar (~2.4×)** → a logged tuning-PLAN finding (NOT a build failure, NOT a reason to move the bar; B-6 ring-fence); the B-4/G-3 four-model sweep shows the `gpt-oss:20b` pin is best on BOTH axes and ~3.5× faster than every alternative (smaller ≠ faster), so the pin HOLDS. PLAN-0016/0017/0018 Done; PLAN-0010 autonomy loop LIVE. Suite **1340 / 2 skipped**.
next_action: **ADR-0018 is Accepted (#242) → NEXT = T2: Code dispatches the Axis-B build PLAN to Cowork (ADR-009 D1).** The PLAN covers: `_goal_gate.py` + its insertion into `stop_continuation.py` (D4 FAIL-OPEN); the `.claude/state/goal.json` schema + tests (D2); the `/goal` command; the `goal-evaluator` subagent (D3 refute-not-bless) + the **SD-1 write-deny hook** narrowing the evaluator's Write to `goal.json`; Telegram wiring; and a verification-rigor case-coverage matrix (D7). After Cowork drafts + Cray ratifies the PLAN → **T3** (add the autonomy-triggers **V-row**) → **build**. Non-blocking / verify-at-execution: **OQ-8** (plugin packaging, MS-S1 local evaluator, blocking-mode promotion, PR-merge gating, auto-declared goals) + **VX-1..3** — none gates T2. · **Prior — ADR-0017 arc is FULLY COMPLETE — OQ-B resolved (#239, session 51); no governance thread is currently blocked or in-flight.** Remaining ADR-0017 OQs are both non-blocking future candidates, owner's pick when picked up: (1) **OQ-A** — migration backlog (non-binding future-PR candidates surfaced by the alignment); (2) **OQ-C** — revisit the Tier 2.6 definition only IF harness-as-plugin packaging is pursued. No probe/host-state owed (session-51 probe artifacts cleaned up, tree clean). Prior: **Skills-as-memory-tier arc is GOVERNANCE-COMPLETE (ADR-0017 ACCEPTED, #236+#237, session 49) — the §50/§57 "draft ADR-017" earmark is CLEARED.** NEXT (open threads): (1) **OQ-B** — skill loader tie-break (same-named project vs global vs plugin skill) is delegated to Code but needs Cray approval for a probe touching global `~/.claude/skills/` (host state — ASK before probing); (2) the deferred **Axis-B verification-loop** track (evaluator subagent + `/goal` Stop-hook gate) from the harness-engineering review. **A restart-bridge handoff is DUE this session** because #237 edited constitutional `CLAUDE.md` (Lesson #5 §1). · **Prior (now RESOLVED — kept for archeology):** Cowork drafts ADR-017 (likely) "Skills as a memory tier" — resolving the 8 OQs in the handoff (`.claude/handoffs/session-49/2026-06-09-2140-code-skills-memory-tier-adr-dispatch.md`): tier placement, `for_llm/` overlap, the canonical-vs-derived rule, authoring ownership, the knowledge-placement decision rule, the precedence ladder, conventions, the migration backlog → **Code commits the resulting `Proposed` ADR per ADR-009 D2 → Cray ratifies.** **Separately/optionally: begin the Axis-B verification-loop prototype** (an evaluator subagent + a `/goal` Stop-hook gate) — the higher-leverage track deferred out of the two-axis harness review. · **Prior context (session 49 earlier — PLAN-0020 Phase 1 OFFLINE is DONE (#232, `8324cba`, `feat(engine):`)): NEXT (Cray-gated, host-state — ASK before warming/running MS-S1): ONE batched campaign = B-3 baselines (text-to-SQL + RAG, REPORTED not gated — heaviest remaining, the conviction artifact) + PLAN-0020 latency levers + the aqua prompt-nudge delta re-run (re-run aqua β with the Step-1 nudge; confirm energy/supply did not regress). Cray must ratify SD-1 (widen supply-α `valid_handlers`→`[hold, inspect]`) + SD-2 (8 s-bar review) BEFORE the relevant gated step; Code does NOT touch grader/dataset until SD-1 ratified. THEN B-5 report finalize + B-6 ring-fence wrap (closes Part B). Sequencing = Cray's call. Standing backlog beyond Part B unchanged: Task (C) Tier-2 real-data path (gate on a design partner, not engineering readiness), PLAN-0010 loop handlers (soak-gated), `status_digest` v2, PLAN-004 Phase C.** · **Prior context (session 48): PLAN-0020 (procedure-path tuning) DRAFTED + on main (#230, `d013b66`, `docs(plans):`) — the B-6 ring-fence follow-up tuning PLAN, status Draft. NEXT was: (1) Cray ratifies it — decide SD-1 (widen supply-α `valid_handlers`→`[hold, inspect]`; needs re-ratify BEFORE any dataset/grader edit) + SD-2 (8 s-bar review); (2) execute — Phase 1 OFFLINE first (fix #2 deterministic `affected_entities` override + its unit test; the aquaculture prompt nudge) via `feat/*`, then the Cray-gated host-state latency-lever + delta re-runs, then REPORT.md fill; (3) B-3 baselines + B-5/B-6 wrap.** · **Prior context (session 48 hardened re-run): PLAN-0019 Part B hardened re-run is COMPLETE (#228, `ec073a4`, `feat(benchmark):`, session 48) — the hardened benchmark DISCRIMINATES as designed on live `gpt-oss:20b` (MS-S1, Cray-approved host-state run); every score VERIFIED via the new `--dump-json` capture, not inferred. β overall 85.8% (103/120; warm companion 89.2% → honest ~86–89%) CLEARS ≥85%; α probe 70.0% (84/120); sanity 100% (198/198); latency p95 22.64 s (~2.8× over the 8 s bar) = a logged B-δ finding (ring-fenced).** **NEXT (1) — author a latency tuning PLAN** from the B-δ finding (p95 22.6 s > 8 s) — **OFFLINE, authored via `plan-drafter`**; levers: trim the `think=True` call-1 pass / batching / a faster-arch small model not yet on MS-S1 / revisit the 8 s bar; **incorporate the supply-α `valid_handlers`-too-narrow observation** (the model picks defensible `inspect` where the dataset pins single `hold`, never the dangerous near-misses — arguably `[hold, inspect]`) as a candidate methodology tweak. **Must NOT reopen ADR-016's primitive shape (ring-fence).** **(2) B-3 baselines** — raw text-to-SQL + a RAG baseline on the same questions (REPORTED, not gated; acceptance = a comparison run + measures recorded) — the heaviest remaining sub-step; hits MS-S1 (host-state). **(3) B-5** report finalize + **B-6** ring-fence wrap (below-threshold = a logged finding → a tuning PLAN that must NOT reopen ADR-0016's shape). All via `test/*` (or `feat/*` where product code) PR(s). Sequencing remains Cray's call. Standing backlog beyond Part B unchanged: Task (C) Tier-2 real-data path (heavy-spend → Cray green-lights), PLAN-0010 loop handlers (soak-gated), `status_digest` v2, PLAN-004 Phase C. · **Prior context (session 47 — Part B hardening COMPLETE, PR1 #224 + PR2 #226):** the benchmark gained the real action vocabulary, the β/α grading split, AND the hard multi-entity/near-miss scenarios + precision checks (#226, `e76977d`, `test(benchmark):` — hard `*-h01..h12` items + `forbidden_primary_keys`/`forbidden_keywords`; 54→66 items/vertical, 84→120 graded breach items); PR1 (#224, `00d7a24`, `feat(benchmark):`) added the `action_type` handler vocabulary ((C) product-complete) + the β-headline / α-probe grading split driven by the handler-determinism finding (procedure path overrides `suggested_handler` with the author's deterministic `step.handler`). The hardened re-run #228 above filled the headline these PRs set up. Standing backlog beyond Part B unchanged: Task (C) Tier-2 real-data path (heavy-spend → Cray green-lights), PLAN-0010 loop handlers (soak-gated), `status_digest` v2, PLAN-004 Phase C. · **Prior context (session 47 earlier — #224, `00d7a24`, `feat(benchmark):`):** Part B hardening PR1 of 2 (handlers-first) — the real ontology action-handler vocabulary ((C) product-complete) + the β-headline / α-probe grading split, driven by the handler-determinism finding (procedure path overrides `suggested_handler` with the author's deterministic `step.handler` → handler-selection rides its own α probe lane, not the β headline); suite 1340/2 at that point. · **Prior context (session 46 — B-β + B-δ COMPLETE):** B-β (#217 scaffold + #219 fill + #220 calibration/run) headline ~98–100%, sanity 100% (162/162), caveated as a FLOOR; B-δ (#222) latency p95 19.2 s OVER the ≤8 s bar (~2.4×) = a logged tuning-PLAN finding, and the G-3 four-model sweep shows the `gpt-oss:20b` pin HOLDS (best on both axes, ~3.5× faster than every alternative; smaller ≠ faster). NB B-δ CORRECTED the prior "qwen3.x = NOT_JSON" note — qwen3.6:35b DOES emit valid structured JSON on this build (MS-S1 Ollama now 0.30.6). · **Prior context:** **→ SESSION 43 (new session) — see handoff** `.claude/handoffs/session-43/2026-06-08-1144-code-session43-procedure-engine-a-epsilon.md` (A-ε = action-step adapter + goal injection; **decide the propose/execute fork before building**, then A-ζ/A-η → HARD GATE → Part B). · **Prior context (SESSION 42 reconcile):** `.claude/handoffs/session-42/2026-06-07-1537-code-session42-procedure-engine-kickoff.md` (locked design shape v3.3 + the two Cray-gated next items). **→ Earmark PLAN-0019 = Phase 1 "Core Procedure baseline"** — the `Procedure/Step/PipelineRun/Agent` runtime + a linear set-valued orchestrator over {query, evaluate, action, human_task} + one hand-authored example procedure per vertical (e.g. aquaculture "Morning Pond Health Round"). Earmarking PLAN-0019 = **G2 always-pause → needs explicit Cray approval** of the number + scope before any `docs/plans/0019-*.md` Write. **Still pending from the kickoff agenda: Thread 2** — empirical gap-testing on synthetic/ungated data (no real data, no PDPA/heavy-spend gate) — and the **per-procedure local-LLM model-selection benchmark** (closes evidence-gap G-3) folds into Thread 2; both feed/shape PLAN-0019's acceptance measures. · **Prior context (Thread 1 now LANDED as ADR-0016):** the session-41 kickoff named two Cray strategy threads gating the PLAN-0019 decision — **(1)** expand the action layer to `anomaly AND normally→action` as a reusable cross-vertical engine (DONE → ADR-0016 Accepted, #190); **(2)** deep-research + **SIMPLE TESTING** to fill the evidence-gaps (benchmark OUR stack vs text-to-SQL/RAG on synthetic ops data; measure local-LLM latency + build-cost) — still open as Thread 2. **Two Cray strategy threads now GATE the PLAN-0019 decision:** **(1)** expand the action layer from `anomaly→action` to **`anomaly AND normally→action`** — governed **agentic** execution of a vertical's NORMAL operating workflow (sequence captured from stakeholder workflow interviews) as a **reusable cross-vertical engine** (likely its own ADR; on-thesis with our safe/human-gated agentic posture vs the agentic "reliability gap"). **(2)** deep-research + **SIMPLE TESTING** to fill the 4 evidence-gaps — benchmark OUR stack vs text-to-SQL/RAG on **synthetic** ops data (gives our own numbers; **ungated — no real data, no PDPA/heavy-spend gate**) + measure local-LLM latency + build-cost via the `new-vertical` demo. Work threads 1+2 THEN lock PLAN-0019 scope (Code leans option **B**; but thread-2 testing may BE PLAN-0019, reframing it from Tier-2-real-data to empirical-validation). Earmarking PLAN-0019 = **G2 always-pause** → explicit Cray approval first. · Prior context: **Dual-track impl-approach research DONE + RECONCILED** (Cray-routed 2026-06-06) — comparing vero-lite's ontology/semantic-layer vs RAG/GraphRAG · text-to-SQL · semantic/metrics layer · fine-tuning/agentic, for a stakeholder **conviction** artifact (the product = an implementation service connecting a customer's structured+unstructured data to LLM/Agentic AI). **Code track DONE** → `docs/research/private/2026-06-06-impl-approach-vs-alternatives-code.md` (private/gitignored; `deep-research` skill, 25 sources / 23 verified claims): text-to-SQL collapses on real enterprise schemas (o1-preview 91→21%, GPT-4o ~0% on BEAVER), a semantic layer adds +17–23pp + flips the failure mode silent-wrong→explicit-error (= our "no data" answer), Dialpad/Palantir endorse the *principle* (LLM-as-planner; Ontology nouns+verbs+human-gate). **Honest caveats:** vendor-biased headline numbers + NO direct benchmark of our exact codegen-YAML stack + NO build-cost / local-LLM / RAG-coexistence numbers. **Cowork track DONE** (`...-cowork.md`, authored independently, **strongly convergent** with Code — same headline numbers + same framing). **Reconciliation** → `docs/research/private/2026-06-06-impl-approach-reconciliation.md`: seams (a) A-vs-D tied-at-5 / (b) keep A unstructured=weak / (c) primary-vs-directional source split — all resolved; **4 evidence-gaps confirmed** (no direct benchmark of our stack; no build-cost / local-LLM / RAG-coexistence numbers) → **task (C) sub-questions**. **AWAITING CRAY:** disposition per ADR-009 D2 (keep private vs lift a sanitized synthesis vs spin the gaps into a committed task-(C) feed — Code recommends the task-(C) feed), conviction-deck greenlight, + a flagged **K-2 harness observation** (Cowork's `.claude/` write-block was not enforced this session — lifted or situational?). — Then the standing backlog: **Task (C)** deep-research the Tier-2 real-data path (real `DataAdapter` CSV/DB/API, dbt/SQLMesh mapping layer, PDPA-safe local-LLM-only ingestion) — heavy spend → Cray green-lights; plus the standing backlog — PLAN-0010 loop handlers (soak-gated), `status_digest` v2, PLAN-004 Phase C, PLAN-0012 Phase 2 (gated). **Highest leverage stays Cray-side: run the live "build your own vertical" demo for a design partner (runbook §5b), and design-partner outreach.**
head_commit: 1be60f7
