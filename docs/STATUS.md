...
> _Rotation note (session-124 reconcile, 2026-07-13, `docs(status):`):
> frontmatter bumped to `head_commit 4a4c444` (session 124); a new s124
> Current-Focus block was PREPENDED. **NORMAL reconcile** (no size pressure —
> comfortably under the R1 ceiling): with s124 prepended, Current Focus held 5
> sessions (s124 + s123 + s122 + s121 + s120), so the OLDEST — the whole
> **session-120** block (`threshold_field` per-entity band shipped, procurement
> `judge_stock` per-part `reorder_point`, #703/#704/#705) — was rotated OUT to
> keep the 4-session window (now s124 + s123 + s122 + s121). Recent Decisions
> rotated its OLDEST row (2026-07-10 flaky-DB-test isolation, #678/#679/#680) to
...
> Prior rotation notes (through the session-123 reconcile) are consolidated into
