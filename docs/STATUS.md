...
session: 218
current_batch: "s218 — three PRs (#1099, #1100, #1101), 0 open. ADR-0036 RATIFIED (Cray, typed), and its D6 follow-on PLAN-0103 drafted + merged Draft the same session. Cray typed five LOCKED content calls; seven SD slots remain open in the PLAN."
current_actor: code
blocked_on: "Nothing is BLOCKED, but execution of PLAN-0103 is gated: SD-1 (what authorizes per-system DB posture), SD-2 (did s218 fire ADR-0036 OQ-3's fleet trigger, and the bring-up order), and SD-3 (the three published tab sets + default tabs) must be ruled by Cray before any Step starts. SD-4..SD-7 can be ruled alongside execution."
next_action: "Cray rules PLAN-0103's SD-1/SD-2/SD-3 (each carries a drafter recommendation + the evidence behind it; SD-3 also carries the second-assisted-system consequence). Then Step 1 (confirm-and-branch on portal-repo existence) and Step 2 (kill PUBLISHED_EXCLUDED_VIEWS for a server-declared per-system view set, walking all 11 isPublished() consumers) are the natural first moves — both offline, no host-state. Ungated Code work if a smaller bite is wanted: BUILD D-4 option (a) (direction ruled s217, four seams in nl_query.py, nothing built); the public one-pager (stream 4, all four sources on disk); versioned font URLs + pin OLLAMA_KEEP_ALIVE (both XS, the latter a drift-guard only). Edge cache-purge still needs a Cloudflare API token = new secret + host-state; MS-S1 headroom is PLAN-0103 Step 9 and is UNMEASURED."
head_commit: f2731be
recent_commits: [f2731be, 9abbfa4, 57f2f7b, fee2ed2, 1a6e29b, 381cd3f, 0ff622f, 6f4cf80, f54d63b, 3ad9522]
---
...
> **Session 218, 2026-08-09 (head_commit `36e5735` → `1a6e29b`) — one PR merged
> (#1099), 0 open. Theme: ADR-0036 is RATIFIED, discharging the one item that was
> Cray's and that gated the highest-value open work.**
>
...
>
> **The verification is behavioural, not cosmetic.** A green test only says the
> expectation set matches; it does not say the gate now guards the file. Probe, with
> the pass/fail read fixed **before** the run: `gate.evaluate(ADR-0036)` → **DENY**
> (was allow), beside two live controls in the same run — `0014-WITHDRAWN` →
> **allow**, Accepted `0035` → **DENY**. Without those controls the DENY is equally
> consistent with "the gate denies every ADR path", which would prove nothing. Gate
> at CI scope: `ruff format --check` clean (616) · `mypy --strict services/`
> Success (133) · **3906 passed / 8 skipped / 0 failed**.
>
> **What it unblocks — and it was taken the same session.** ADR-0036 designs the
> "open the demo and pick which vertical" portal, which *is* a landing surface, so
> the landing PLAN had to be drafted **downstream** of this ruling rather than
> beside it; building the shell first risked building the wrong shell.
> **PLAN-0103 is now drafted and merged `Draft` (#1101)** — `plan-drafter`
> authored it, Code R2'd, Cray ratifies. Two findings from the pre-dispatch
> fact-check reshaped it: `isPublished()` has **eleven** consumers across eight
> files (so "change the excluded-views array" was never the whole change — the
> drafter proposes **eliminating** the constant), and **fleet publishing Tab C
> makes a second assisted system** against one Ollama, contradicting the premise
> ADR-0036 D5 wrote its aggregate-inference posture on. **Seven SD slots are
> open; SD-1/2/3 gate execution.**
...
|------|----------|-----------|
| 2026-08-09 | **s218 — PLAN-0103 DRAFTED `Draft` (#1101): vero-lite's side of the multi-vertical portal, ADR-0036's D6 follow-on.** 578 lines, 10 Steps, 10 ACs, **7 Cray SD slots — SD-1/2/3 gate execution, so no Step starts before they are ruled.** Cray typed five LOCKED calls: per-system DB posture (fleet gets Postgres; energy + procurement DB-less) · fleet is the **leftmost card** · Tab E's narrative moves portal-side · bilingual TH/EN · **three demo personas, one API key each**. Built around the hard boundary — **zero portal-repo files, no landing page in vero-lite**; card copy is one-system-per-file with **no roster** (AC-5 guards D2's no-second-registry rule). 🔴 Two findings reshaped it: **`isPublished()` has ELEVEN consumers across eight files**, so the drafter proposes **killing `PUBLISHED_EXCLUDED_VIEWS`** for a server-declared per-system view set rather than a third profile value; and **fleet publishing Tab C makes a SECOND assisted system** on one Ollama, contradicting the premise ADR-0036 D5 wrote its aggregate posture on | `f2731be` (head_commit) / [#1101](https://github.com/CrayJThiemsert/vero-lite/pull/1101) / `docs/plans/0103-portal-landing-and-per-system-published-profiles.md` |
| 2026-08-09 | **s218 — ADR-0036 RATIFIED (Cray, typed): a deployed vertical instance IS a system (#1099).** Accepted as drafted — D1 scope (a), D2 vertical-as-system + the `oct-<vertical-id>` label convention, D5 per-vertical profile ownership (N ≤ 3 duplication taken deliberately), OQ-1 = retire the bare `oct.` label. ADR-0035 D1–D4 untouched; the D4 reopening trigger does not fire. The ratifying edit **also removed `0036` from the gate's set-equality assertion, same commit** — the entry was an in-flight marker, not an exemption. Verified behaviourally, not just green: `gate.evaluate(0036)` → **DENY**, beside live controls `0014` → allow and `0035` → DENY. **Unblocks the landing/framing-layer PLAN**, whose ordering prerequisite this was | `1a6e29b` (head_commit) / [#1099](https://github.com/CrayJThiemsert/vero-lite/pull/1099) / `docs/adr/0036-vertical-as-system-multi-vertical-demo-portal.md` |
...
- [ ] **PLAN-0103 — vero-lite's side of the multi-vertical portal. DRAFTED s218 (#1101), `Status: Draft`, nothing built.** ADR-0036's D6 follow-on: per-system published profiles + the landing/framing **content spec**. 10 Steps, 10 ACs, **7 SD slots**. 🔒 **SD-1/SD-2/SD-3 gate execution — no Step starts until Cray rules them:** SD-1 = what authorizes per-system DB posture (drafter reads ADR-0036 D5's "PLAN's to finalize" grant as sufficient, no separate ADR owed); SD-2 = did s218 fire ADR-0036 OQ-3's fleet trigger, and is bring-up order system-number or card order (**three independent axes — card order ≠ system number ≠ bring-up order**); SD-3 = the three tab sets + default tabs, which also carries the second-assisted-system consequence. SD-4 (persona-key delivery) · SD-5 (fleet Tab H seed vs visitor-driven) · SD-6 (dev-console Tab E) · SD-7 (bilingual depth, EN role renderings, CTA) can be ruled alongside execution. **Read the PLAN, never a restatement:** `docs/plans/0103-portal-landing-and-per-system-published-profiles.md` (§The hard boundary · §Surfaced decisions). ⚠️ Two things a future reader must not re-derive: the **hard boundary** — ADR-0036 D1 + ADR-0035 D4/L5 make the `portal.` landing surface, ingress map, Access policies and domain **portal-repo property**, so this PLAN builds no landing page here and ships a spec instead; and `isPublished()` has **ELEVEN consumers across eight files**, so any step touching published-ness must walk all of them (`tools/excision_scope.py` + the `excision-scope` skill).
