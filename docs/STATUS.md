---
last_updated: 2026-08-26T16:10:00+07:00
session: 256
...
next_action: "🟢 DEPLOYED — 0113 + 0114 are LIVE on the fleet system (Cray's typed go, s256). Host `ee41b55` → `dd4228f`; image `0fc679cf` → `880307365d7f`, id-identical across machines; only `app` recreated, then `cloudflared` force-recreated for the new ingress row; `DEMO-STATE: PRISTINE` unchanged; rollback tagged `:prev`. Record: `docs/logs/2026-08-26-s256-fleet-deploy-plan0113-plus-0114.md`. ⚠️ STILL OPEN: the visitor WALK through the public edge — PLAN-0113 AC-9 and PLAN-0114 AC-5's live half are now blocked only on that, not on the deploy. Needs the published hostname, which is deliberately not in this repo (ADR-0036 D2)."
head_commit: 6882b8b
...
- [ ] **🔴 `tools/probe_battery/` restores CONTENT but not file MODE — and its own byte-identical check cannot see the gap.** Measured s256 mid-deploy: the three files this session's batteries mutated (`persistence.py`, `action_step.py`, `orchestrator.py`) came back **`0600`**. `git status` is blind (`core.fileMode=false`), the suite is blind (tests read as the owner), CI is blind (a fresh clone gets git's `100644`) — but `docker build` copies the mode faithfully and the container runs as `uid=999`, so **the built image could not import its own engine** (`PermissionError` on `persistence.py`). Only DEPLOY.md §2a's in-image hash check found it, one step before the host would have been touched. Fixed by hand for this deploy; **the driver is still wrong** — its restore must assert mode as well as bytes.
- [ ] **🆕 The visitor WALK through the public edge is still owed** — PLAN-0113 **AC-9** and PLAN-0114 **AC-5's live half**. The deploy is done and the ingress row is verified present on the host's bind-mounted `config.yml`, but **no public request was made**: that needs the published hostname, deliberately absent from this repo (ADR-0036 D2). Walk = a mid-band acceptance (single-proposal gate) **and** a sub-ceiling one (empty gate → Acknowledge → completed), plus the demo reset still reading `PRISTINE`.
