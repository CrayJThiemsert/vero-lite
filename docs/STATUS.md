---
last_updated: 2026-08-28T16:11:55+07:00
session: 260
current_batch: "s259-260 — ONE PR merged (#1309, five commits): Cray ratified a 7-phase FDE readiness programme and phases 0/1/1.5/1.6 shipped. Phase 1.5's 'keep gpt-oss:20b' verdict is SUPERSEDED by the phase-1.6 matrix — every challenger error was a deadline cut, and turning the reasoning pass off lifted qwen to beta 80%."
current_actor: code
blocked_on: "Nothing blocking. Main green at `7306f17`; 0 open PRs. Owed: the phase-2 dry-run model decision is REOPENED and no model is bound · the `check_ac_consistency.py` backwards-lookback gap · PLAN-0116's three SDs · the 1.8 GB worktree sweep · PLAN-0109 · PLAN-0108."
next_action: "Cray's call: the FDE programme's next step (repeat-matrix on the three surviving configs plus the never-run fleet set, then structured_output_health), or a non-programme item — PLAN-0111 has the best design-readiness in the repo. PLAN-0109's drafter round must repair FOUR ruled defects; the Active-TODO row now names all four."
head_commit: 7306f17
recent_commits: [7306f17, 6be4db5, a6a4512, 10e7b00, 0703a5c, 704cbe6, 5292c99, d78eebe, 1904caa, 1993bda]
---
...
- [ ] **🆕 CRAY'S CALL — the phase-2 dry-run MODEL DECISION is REOPENED; nothing is bound.** Phase 1.5 read *keep `gpt-oss:20b`*; phase 1.6 superseded it by measuring every challenger "error" as a **deadline cut**, not a model failure. 🔴 **No winner can be declared on what exists** — all six matrix cells are a **single repeat**, against a measured **45% flip rate** on the incumbent (nine of twenty items changing verdict between identical runs, larger than most of the gaps found). ⚠️ The programme's ratified next step is a **repeat matrix on the three surviving configs** (`gptoss/skip`, `qwen/think_off`, `qwen/skip`; three repeats each, joined so the flip rate prints beside the accuracy) **including the fleet set** — the only set that can separate *"picked a defensible action"* from *"matched the pinned string"*, and which **has never been run**, so it is not yet an oracle (§8). Then `structured_output_health`. ⚠️ `energy-002` and `energy-008` were cut in **both** thinking modes; Cray asked those logs be kept, because two items reproducibly slow across configurations is a signal about specific inputs. 🔴 **Every live run needs a NEW typed §8 go** — the three given in s259 do not carry forward. **Read:** `benchmarks/model_compare/RESULTS-1.6.md` + the gitignored carrier `docs/strategy/private/2026-08-28-fde-readiness-program.md`.
...
- [ ] **🆕 CRAY'S CALL — should R2 cap the Active TODOs *COUNT*, and govern the Recent Decisions trailing ledger?** 🔴 **RE-MEASURED s260, and the question is no longer optional: STATUS closes this reconcile with well under 1 KB of headroom below R1's 65,536-byte HARD ceiling** (a deliberately coarse figure: an exact byte count written into the file it measures has no fixpoint) — so the next reconcile fails the pre-commit guard unless something is capped, and rotation alone will not save it (this one rotated two slices and still grew). Active TODOs is **31,703 B — 48.8% of the file across 48 entries**, up from 21,311 B / 43% / 38 entries at s255; every entry is ≤ ~600 chars and fully compliant, because **nothing bounds how many there are**. The RD trailing ledger is **5,547 B and still ungoverned** (s250's cap reached the *Current-Focus* ledger only, now 1,086 B). The four Current-Focus blocks are **2,851–4,055 B**, all inside R2's 4,096 cap — the s256 block by just **41 B**. Capping either **authors a rule**, so Code surfaced it rather than trimming (R6). **Read:** `docs/runbooks/memory-architecture.md` §R2.
