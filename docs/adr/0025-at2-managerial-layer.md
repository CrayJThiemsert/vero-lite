...
_[Citation repaired 2026-07-30 (session 194): the archive pointer in the closing paragraph below originally read `docs/status-archive/2026-h1-status.md:1462-1466`. It rotted at the **session-144 archive split**, when the base archive file was re-chartered as the rolling recent-window head and the session-84 block migrated into the lettered sibling `2026-h1c-status.md` (that session's repair sweep fixed three pointers of this shape and missed this one; the session-193 R4 split is **not** the cause — it preserved every section heading). Repaired by citing the sibling **by section heading with no line numbers**, per the house lesson "cite section headings; they survive an edit, line numbers do not": a heading anchor survives future splits — at worst the sibling filename changes again and the heading remains the greppable key across `docs/status-archive/` — so this repair does not re-enter the rot class it fixes.]_
