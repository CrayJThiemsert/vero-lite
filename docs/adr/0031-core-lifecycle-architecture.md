...
| **Governance gate** | The criterion **vocabulary is no longer engine-owned** (row re-verified 2026-07-21): a vertical **declares** its own `rule_gate` vocabulary — `VerticalProcedures.compliance_criteria: list[CriterionId]` (`spec.py:1680`), pattern-constrained (`CriterionId`, `spec.py:853`), membership-validated at load (`_validate_compliance_criteria`, `spec.py:1709`); `ComplianceCriterion` is retired from engine code (retirement note in place, `spec.py:870-878`) — a new AT-2 vertical ships its `rule_gate` with **zero engine diff** (PLAN-0087 AC-1 proof pair). Adding a new gate **shape** still touches ≥4 coordination points — PLAN-0087 did not reduce this: the `GateKind` member (`spec.py:202-216`), the typed content model + `AT2Governance` union arm (`spec.py:1141-1143`), a resolver module (`doa_tier.py` / `scored_rule.py` / `rule_gate.py`), and the wiring in `governance_step.py` + `derive_governance_todo` (`draft.py:293`) | **Criterion vocabulary: shipped — see PLAN-0087** (declaration-as-data, typed, closed per vertical, load-gated — tripwire 4 honored). The full **governance-gate plugin unit** — content-model + resolver + obligation-contributor registered as ONE typed unit, plus **decision-as-data** (the DOA/scored-rule content is already DMN-shaped: the decision table becomes a versioned, auditable artifact separate from control flow) — **remains pre-designed** for gate-shape pressure that has not arrived: the N=4 instances pressed the vocabulary axis only (subset-with-honest-deferral, in-authority per D4.2 + the Negative-section deviation clause; this row amendment is that deviation record — the PLAN-0077 row-1 precedent) | **Fired and adjudicated three times** (ADR-0025 D7's re-trigger): N=2 (PLAN-0074 — re-evaluated, deferred), N=3 (PLAN-0081 — re-armed), N=4 (PLAN-0086 — **cancelled, extraction due**; Cray, typed, 2026-07-21). The vocabulary axis is **discharged** (PLAN-0087); the remaining axis — the procedure-aware `ExecutorFactory` (`sod_steps`/`stamp_steps` keyed on `(procedure_id, step_id)`) — stays open, tracked at **PLAN-0076 Step T1**, its triggers armed. ⚠ note updated (`superseded by new info` — accurate at 2026-07-14 ratification, see OQ-4): D7's promised CI count-assertion **has since been built** — PLAN-0074 AC-11, `tests/services/engine/procedures/test_at2_signature_retrigger.py` (its module docstring owns the marker); it is the marker that fired at N=3 and N=4 |
