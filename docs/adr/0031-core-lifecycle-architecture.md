...
|------|------|------|------|
| **Steps / StepKinds** | Closed enum (`spec.py:54-60`) + per-vertical executor registry. Known missing member: a `transform` StepKind — the procurement intake `_SeedQuery` stays execution-bound precisely because derived fields would require "inventing a transform StepKind (out of scope)" (`verticals/procurement/hero_demo/run.py:133-159`); ADR-016 Q4 already names "a downstream transform step" as where derived/computed fields belong (ADR-016 :1094-1097) | A **typed declarative transform grammar**: a bounded op-set — `rename / coerce / unit_convert / map_value / extract / normalize / default` — plus a `derive` op restricted to a **whitelisted expression grammar** (arithmetic / concat / compare; **never** arbitrary eval — tripwire 1). NOT an open worker kind (tripwire 2). Exact op-set + grammar surface: the transform PLAN (OQ-1) | Case 1 = the `_SeedQuery` derived-field residue (already on disk). Fires at case 2: a **second** vertical with messy-intake derived fields |
