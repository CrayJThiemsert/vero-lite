...
## Amendment (2026-07-15, RATIFIED — Cray, session 132): D4 gains a FOURTH fail-closed condition — tier-authority enforcement at the gate (implementing PLAN: PLAN-0075)

> **In-place amendment discipline.** Extends, does **not** reverse or renumber — the ADR-0022 inline-amendment / ADR-016 D2-Amendment precedent. Status **stays Accepted**. This amendment **completes** the guarantee D5 and D6 already state — the executor "routes / **suspends to the resolved approver** for that tier" — it does not reverse any decision. Context (verified vs `origin/main` ff84d9a, PLAN-0075 §The reproduced gap): the shipped run gate enforced only the generic SoD step→role map (`principal_sod.py:176`); **no path enforced that the acting approver holds the ladder-resolved tier role**, so a lower-tier approver could resolve a gate the ladder routed to a higher tier — and the run-time `GovernedDecision` named an authority who never acted.

### D4, condition (iv) — the tier-authority fail-closed trigger (ADDITIVE)

D4's fail-closed list was exhaustive at three conditions (i)–(iii). It now gains a **fourth**, enforced at `resolve_gated_step` by a **pure** tier-authority run-check that sits ADDITIVELY beside `check_principal_sod` (LOCKED #2 — added to, never replacing or weakening it):

> **(iv)** on a gated step carrying AT-2 **authority** content (`doa_tier` / `severity_tier`), the acting approver does **not hold the ladder-resolved tier role** (the `required_role` on the step's persisted authority verdict) — **OR** the step declares authority `governance_content` but carries **no persisted authority verdict at all** (the missing-verdict case: without it, an executor binding that bypasses the governance wrapper would pass silently — PLAN-0075 AC-3).

Fail closed = the gate refuses BEFORE any approve/execute (no handler fires, no governed verdict is emitted), the refusal is durably audited, and the check renders / routes / **blocks** only (LOCKED #3 — the ADR-0007 approve→execute gate stays the only write path).

**Residual risk (recorded honestly — 2026-07-15 specialist review):** tier-role enforcement trusts the run-time severity/spend **derivation**, which is human-authored code under change control — supply_chain's derivation constants (`_DOSE_LADDER` + its `_TOP_SEVERITY` top band, `cold_chain_assess.py:71-77`) are provenance-hashed into the run's governance snapshot (PLAN-0075, provenance-only: mid-flight tamper-evidence + which-derivation-governed-this-run, NOT a new-run guarantee), but procurement's runtime ฿ derivation is **not yet pinned** — tracked as the F-PIN follow-on, whose proper form is declaring the derivation as governance data (ADR-0031 D3 row-1), at which point the existing pin covers it.

### The `RoleId`-rank stance (PLAN-0075 SD-1 = (a); Cray, s132)

Rank over `RoleId` stays **authored DATA**: vero-lite adds **no engine rank primitive** over `RoleId` (which remains a free string per D2). The enforcement predicate is **exact-match** — `verdict.required_role ∈ principal.roles`. Where a senior should approve downward, that authority is **expressed by authoring cumulative role sets** in the vertical YAML principals (declaration-as-data, ADR-0031 D2 spirit). Failure posture: **over-strict, never unsafe** — it refuses a senior; it never admits a junior. **D5 reconciliation (so this ADR is not self-contradictory):** D5's phrase "run-checkable against the resolved role rank" (the `EmergencyWaiverPolicy` strict-escalation) is to be read under this stance as rank-as-authored-data — it does **not** imply, and never built, a rank primitive; the waiver strict-escalation run-check remains deferred to its own future PLAN.

### The multi-verdict fan-out rule (PLAN-0075 SD-2 = (a) source + (i) rule; Cray, s132)

The enforcement reads the **persisted engine-written verdicts** on the gated step's audit (`audit["doa_tier" | "severity_tier"]` — a list, one verdict per input entity), and the acting principal must satisfy **EVERY** verdict on the resolved step. Under non-cumulative authored roles a mixed-tier verdict set is therefore satisfiable by no single principal and **fails closed** — acceptable now (every shipped authority resolution carries exactly one verdict — hero seed cardinality 1, `hero_demo/run.py:347`; the mixed-tier case is a latent-regression guard, PLAN-0075 AC-9) and correct later (cumulative role sets are the authored remedy).

### The audit tie names the ACTOR (PLAN-0075 SD-6 = (a); Cray, s132)
