...
**Status:** **Accepted** — ratified by Cray 2026-09-08 (session 286), typed, together with the instruction to begin Steps 1–4. All nine Surfaced Decisions already carried Cray's typed ruling from s274; this flip ratifies the PLAN itself, which had stayed `Draft` while its SDs were decided. **Steps 1 and 2 are COMPLETE** — Step 1 in [#1429](https://github.com/CrayJThiemsert/vero-lite/pull/1429) (`AC-1`, `AC-8`; battery 8 claims / 7 witnessed RED / GAPS 0) and Step 2 in [#1430](https://github.com/CrayJThiemsert/vero-lite/pull/1430) (`AC-2`; battery 18 claims / 17 witnessed RED / GAPS 0), both green at true CI scope. **Step 3 is next** and is the largest change here; it carries one item nobody has ruled — **SD-1.1**, a client shared by two classes — which Step 3 must surface rather than decide.
**Owner:** both (Claude Code executes; Cray adjudicates the Surfaced Decisions and every live arm)
**Created:** 2026-09-03
**Revised:** 2026-09-08 (session 286) — status flipped `Draft` → `Accepted` on Cray's typed instruction; Steps 1–2 recorded complete. §3's call-site inventory defect is **corrected in the same session** — see the `was an error` (s286 revision) block beneath that table. It was drafted by the in-harness `plan-drafter` and landed in its own PR, deliberately not bundled into this ratification. The canonical, guard-backed inventory remains [`docs/conventions/llm-workload-taxonomy.md`](../conventions/llm-workload-taxonomy.md) §2.
...
> ⚠️ **s286 — read before using this table.** The heading's count, the sentence above and the numbered rows carry a verified defect, corrected in the `was an error` block beneath the table: row 13 is **not** a `.chat(` site, and the site now at row 15 was missing. The operational, guard-backed inventory is [`docs/conventions/llm-workload-taxonomy.md`](../conventions/llm-workload-taxonomy.md) §2 (shipped by Step 1); this table is kept as reasoning lineage.
...
| 13 | `action_step.py:422→432` (`generate_judgment`) — ⚠️ *`was an error`, s286: a **J workload**, not a `.chat(` site — the file has zero `.chat(`; see the block below* | J | **stubbed in every vertical except procurement** | via `structured.py` |
| 14 | `scaffold.py:676` | A | yes (CLI) | no |
| 15 | `gate_advisory.py:160` (`_entry`, bare) — *added s286: the site the original grep dropped* | N | **LLM arm unreachable** — no construction passes `client_factory` (`:156` guard); the builder itself is live (liveness notes below) | no |
| — | `.claude/hooks/_sonnet_classifier.py:707-733` | **G** | yes | n/a |

> ⚠️ **`was an error` (s286 revision) — the table was drawing from two different sets, and the count concealed it.**
> **Claimed:** the heading and the sentence above say this table enumerates the `.chat(` call sites under `services/` — 14 of them — verified by grep. **True:** there are 14 `.chat(` sites under `services/`, but **not the 14 this table numbered**. One error in each direction, cancelling in the count:
> - **Row 13 is not a `.chat(` site.** `services/engine/procedures/action_step.py` contains **zero** `.chat(` — a grep of `\.chat\(` over that file returns no hit; its LLM-related lines are the `generate_judgment` import (`:59`), the `client = self.client_factory(...)` construction (`:422`) and the `await generate_judgment(client, event, ...)` call (`:432`). It is a real **J workload**, but it reaches the model only through `structured.py::generate_judgment`, whose two calls are already rows 7 and 8 — so as a *site* it double-counts them.
> - **`gate_advisory.py:160` is a `.chat(` site and was missing.** `services/engine/procedures/gate_advisory.py:160-173` is a bare `client.chat(` (messages only — no `response_format`, no `think`) inside `_entry`, behind the `client_factory is not None` guard at `:156`. It appeared only in the liveness notes below, never in the numbered table. Now row 15 (**N**; the LLM arm is unreachable exactly as the liveness note records). 15 numbered rows minus one non-site = 14 sites, matching the measurement.
> **How verified (s286):** (i) `\.chat\(` grepped over `services/**/*.py` — exactly 14 hits: `run_query.py:511,:574`, `nl_query.py:678,:691,:1225`, `llm/intake.py:182`, `llm/structured.py:245,:260`, `procedures/generator/pipeline.py:236,:245,:480`, `procedures/gate_advisory.py:160`, `action_verification.py:294`, `scaffold.py:676`; (ii) the same grep over `action_step.py` — no hit; (iii) `gate_advisory.py:140-184` and `action_step.py:410-434` read directly; (iv) the AST walk in `tests/services/engine/llm/test_workload_inventory_contract.py` (Step 1, AC-1) enumerates the same 14 from the tree. The record does not say whether the original "verified by grepping" was run and mis-transcribed or not run against the table as written; either way the sentence over-claimed.
> **The contract, made explicit — this, not the two rows, is the fix.** The rows had been drawing from two sets at once: *`.chat(` call sites under `services/`* (what the heading, the grep and the guard test enumerate) and *LLM workloads reached indirectly or from outside `services/`* (`action_step.py`, which reaches the model only through `structured.py`; the `_sonnet_classifier.py` hook, which is outside `services/` altogether). Conflating the two sets is what produced the defect, not merely the occasion for it. **This table enumerates the first set only.** The second set is a separate, named list — the canonical artifact keeps it as *"Two workloads this table does NOT guard"* — and row 13 and the `—` hook row above are members of *that* set, retained here unmodified as lineage.
> **Which artifact is canonical, and what keeps them in sync.** [`docs/conventions/llm-workload-taxonomy.md`](../conventions/llm-workload-taxonomy.md) §2 is the canonical inventory — keyed `path::function::shape` (Cray, typed, s286), with the guard test reading **that file** (test `:38`), enumerating from the tree, not from a constant (test `:8-12`), and admitting only `G / S / J / N / A` (test `:42`). This PLAN's table is **reasoning lineage, not a second operational copy**: it is corrected here once, for the record, and is **not** re-synced when a site is added or removed — a new site reddens the guard against the conventions file, never against this one. The two stay in agreement by there being only one that anything reads.
> **What survives the correction.** The count of 14 `.chat(` sites — cited in §2 (*"twelve of the fourteen … no generation accounting"*), SD-3 (*"12 of 14 sites inherited it"*) and SD-3's interaction note — is unchanged, and so is 12-of-14: the site swapped in (`gate_advisory.py:160`) has no accounting, like the entry swapped out. No ruling, Step or AC rested on *which* 14; this is a table defect, not a decision defect.
