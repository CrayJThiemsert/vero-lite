...
**Created:** 2026-09-03
**Revised:** 2026-09-08 (session 286) — status flipped `Draft` → `Accepted` on Cray's typed instruction; Steps 1–2 recorded complete. ⚠️ **§3's call-site inventory carries a verified defect that is NOT yet corrected here** — see the note in that section's heading area and in [`docs/conventions/llm-workload-taxonomy.md`](../conventions/llm-workload-taxonomy.md) §2. Correcting it inline is a further `docs/plans/` write and is not bundled into this ratification.
