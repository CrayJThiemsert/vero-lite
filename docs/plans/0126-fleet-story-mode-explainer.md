...
**Closeout (2026-09-16):** Steps 0–13 are done — implementation in #1499 and #1500, the Artifact published (Step 9), and the fleet system deployed under Cray's typed go (Step 13; AC-10 `pre=404 post=200 csp=present`). **AC-1 … AC-11 are ticked**, each on a WITNESSED probe or a printed reading (§4); AC-3's and AC-7's artifact lists were amended under option (C), which Cray approved (§4.1 ✎ closeout amendment). **Completion awaits Cray's ratification**; after that, `git mv` to `docs/plans/done/` goes in its own `docs/*` PR (Step 14).
