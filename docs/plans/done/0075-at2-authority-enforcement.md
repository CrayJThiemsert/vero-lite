...
**Status:** **COMPLETE** (session 133) — all 13 ACs shipped + merged to `main`: **core AC-1..AC-12 via #749** + **AC-13 (supply_chain derivation provenance) via #751**; the ADR-0026 D4 amendment (AC-12) merged first (#746). Cray ratified at PR; closed → `docs/plans/done/`. The two tracked follow-ons (F-PIN's remainder + the ADR-0031 D3 gate-plugin seam) live on in **PLAN-0076** (Tracking) with a presence guard-test. — Original SD adjudications, retained for the record: **SD-1 / SD-2 / SD-4 / SD-6 RATIFIED (Cray, session 132)**; **SD-3 = keep NARROW, CONFIRMED (Cray, s132, after a 3-specialist review — with the architect's binding tracked-follow-on condition)**; **SD-5 = ADJUDICATED SPLIT (Cray, s132): defer the bulk of F-PIN; FOLD IN the supply_chain derivation-provenance half (AC-13, provenance-only)**.
