...
| **2** — synthetic adapter | #156 (deterministic) + #160 (LLM) | **Sequencing call:** a deterministic, minimal-but-runnable `synthetic.py` ships first (the command always produces a runnable vertical, CI stays deterministic); the **opt-in `--llm`** MS-S1 draft (richer records, semantically validated, deterministic fallback) layers on top — live-verified against the pinned `gpt-oss:20b` (ADR-0001 — reliably honours the `format` JSON-schema constraint; an initial smoke mis-used the ADR-rejected `qwen3.6:35b`, since corrected). Both render through one shared renderer. |
