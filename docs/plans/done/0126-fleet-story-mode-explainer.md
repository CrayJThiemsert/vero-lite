...
**Status:** Accepted — **ratified by Cray, typed 2026-09-15: "ratify PLAN-0126 แล้วเริ่ม Step 0 ได้เลย"** (the PLAN-0122 `Accepted — ratified by Cray` form). **All seven SDs RULED (Cray, typed 2026-09-15: "SD-1 ถึง SD-7: เอาตามที่แนะนำทั้งหมด"): SD-1 = (a) · SD-2 = (a) · SD-3 = (b) · SD-4 = (b) · SD-5 = (d), confirmed by Cray at PR #1495 review ("SD-5 ยืนยัน (d)", §7) · SD-6 = (a) · SD-7 = (a)** — each recorded inside its own SD in §7 with the options and recommendation kept as drafted. Every Step is unblocked. Ruling the SDs did **not** ratify the PLAN; Cray's separate ratifying act is the one recorded at the head of this line (merged as `Draft` in #1495, flipped here).
...
**Closeout (2026-09-16):** Steps 0–13 are done — implementation in #1499 and #1500, the Artifact published (Step 9), and the fleet system deployed under Cray's typed go (Step 13; AC-10 `pre=404 post=200 csp=present`). **AC-1 … AC-11 are ticked**, each on a WITNESSED probe or a printed reading (§4); AC-3's and AC-7's artifact lists were amended under option (C), which Cray approved (§4.1 ✎ closeout amendment). **Completion ratified by Cray, typed 2026-09-16: "merge #1500, #1496, #1501 แล้ว ratify PLAN-0126 เสร็จ"** — so this file moves to `docs/plans/done/` in its own `docs/*` PR (Step 14). ✎ Of the three PRs that line names, #1500 and #1501 merged (`42b52191`, `a6b51590`); **#1496 is the concurrent PLAN-0125 session's PR and did not merge — it was BEHIND the base branch.** It carries nothing this PLAN depends on. STATUS reconcile stays with that session.
...
**Batteries:** `tests/batteries/plan-0126-*.json` — live since Step 11, in the same commit as the three battery files (page, drift, scenario), as `tools/check_ac_consistency.py` Check 3 requires. No AC box is ticked before its probe. _[As drafted, kept for lineage: the header was deliberately inert until the first battery file existed, because Check 3 errors on a glob that matches no file.]_
...
| `story.js` | the scene, story track, schedule, `evaluate(t)`, controls — an ES module; `import * as THREE from './three.module.min.js?v=160'` (same-origin, so `script-src 'self'` admits it; no import map) | `/story/story.js` |
| `three.module.min.js` | Three.js `0.160.0`, vendored (MIT) — the **minified** build (Step 1 amendment: the unminified `three.module.js` exceeds the repo's 1000 KB large-file hook), Cray-permitted download | `/story/three.module.min.js` |
...
- [x] **AC-1 [check] — the served story loads nothing from outside the origin.** No `http(s)://` or scheme-relative reference in any `src`, `href`, `import … from`, `@import` or `url(` across the authored story files (`index.html`, `story.js`, `story-data.js`, `story.css`; the vendored module is excluded **by name** and the test asserts each authored file exists, so the scanned set cannot be empty); the checker is shown to flag a fixture string holding the stage-1 CDN import. *Artifacts:* `tests/api/test_story_page.py::test_story_files_reference_no_external_origin`, `tests/api/test_story_page.py::test_the_origin_checker_flags_a_known_cdn_import`. *Pass read:* both green; the first prints `refs=N` with N ≥ 4. *Probes:* P1a, P1b. ✅ **Closed 2026-09-16:** `files=4 refs=9 external=0`; the control flagged the stage-1 CDN import; P-A1, P1a, P1b, P1c WITNESSED (`plan-0126-story-page.json`).
- [x] **AC-2 [check] — `/story/` is served under the console CSP on the published profile, unprofiled, and PLAN-0100's fail-loud root stays intact.** With `settings.ui_profile` patched to `published` (G24) and an ASGI client that does **not** raise app exceptions: `GET /story/` → 200, `Content-Security-Policy == main._OCT_CSP`, body contains `type="module" src="story.js`, body contains **no** `name="ui-profile"`, `cache-control` is not `no-store`. Controls: `GET /` on the same client contains `content="published"`; `_profiled_index` called directly on a `FileResponse` to a temp file lacking the anchor raises `RuntimeError`. *Artifacts:* `tests/api/test_story_page.py::test_story_index_is_served_unprofiled_on_the_published_profile`, `tests/api/test_story_page.py::test_root_index_is_still_profiled`, `tests/api/test_story_page.py::test_profiled_index_still_raises_without_the_anchor`. *Pass read:* all green; the first prints `status=200 csp=match profile_meta=absent`. *Probes:* P2a, P2b, P2c. ✅ **Closed 2026-09-16:** `status=200 csp=match profile_meta=absent cache_control=''`; root control `published_tag=True`; P2a–P2h WITNESSED.
- [x] **AC-3 [check] — the edge admits exactly the story paths, anchored, and the table says so.** `config.yml` gains `^/story/$` and `^/story/[^/]+$`; `_EXPECTED_ALLOW["oct-fleet-maintenance"]` gains the same two; every file in `static/story/` fullmatches a story row, `story/` has no subdirectory, and the control path `/story/sub/x.js` matches **no** row. *Artifacts:* `tests/api/test_story_page.py::test_every_story_file_is_admitted_by_a_story_ingress_row`, `tests/api/test_story_page.py::test_the_story_rows_are_anchored_and_the_expected_table_carries_them`, `tests/api/test_story_page.py::test_the_link_and_the_edge_agree_on_every_published_system`. *Pass read:* all green; the first prints `files=6 admitted=6 control_admitted=0`. *Probes:* P3a′ (`P3a2`), P3b′ (`P3b2`), P3c–P3f, P7h. ✎ *Amended at closeout under option (C), approved by Cray, typed 2026-09-16 (§4.1 closeout amendment). The ratified artifact list named the set-equality and anchoring tests of the deploy profiles module plus the admission test, and the ratified probes were P3a, P3b and P3c — the first two addressed a module outside this PLAN's batteries (`was an error (plan)`).* ✅ **Closed 2026-09-16:** `rows=['^/story/$', '^/story/[^/]+$'] files=6 admitted=6 page_admitted=True control_admitted=0`; `table_rows == config_rows`, `unanchored=[]`; the link/edge agreement reads fleet `(declares I+H, admits /story/) = (True, True)` and the other systems `(False, False)`; all seven probes WITNESSED. Live: the edge admitted `/story/` and refused `/story` and `/story/sub/x.js` (AC-10).
- [x] **AC-4 [check] — the data block equals the real ontology, procedure, rules and seed, and every case outcome is the real rule's outcome.** Eleven assertions, one test each, all reading real producers (§3.2): (a) object types = `load_doc(yaml)["object_types"]` keys, set and count; (b) link types = the YAML's `(name, from, to)`; (c) undeclared refs = `type: ref` targets with no `link_types` pair, computed from the file, non-empty; (d) step ids in order = `load_procedures("fleet_maintenance")`'s `governed_repair_approval` steps, `quote_gate` = `rule_gate`/`three_quote`, `approve` = `doa_tier`/`gated`; (e) tiers = the loaded ladder's `(min_amount, approver_role)`; (f) SoD `distinct_steps` and `event_kind` match; (g) `three_quote_threshold_thb == int(THREE_QUOTE_THRESHOLD_THB)`, `min_distinct_vendors == MIN_DISTINCT_VENDORS`, `minor_repair_ceiling_thb` equals every `truck_records()` ceiling (all equal, printed); (h) per case `compute_three_quote(...) == (expected.sourcing_pass, expected.basis)`, `expected.tier_role` = the loaded ladder's tier for the amount, `expected.breach == amount >= ceiling`, `expected.outcome` derived; (i) every `illustrative: false` case matches a seed row in `synthetic._fixture_events()` — the fixture alone, read directly because `operational_events()` overlays the module-level live-case view that `case_projection.refresh(session)` fills, so its content would depend on which tests ran first (G31) — by truck, amount and stamped `three_quote_basis`; the `illustrative: true` case matches none; the count of real matches is printed (`seed_matches=2`); (j) `generate_all(yaml, tmp_path)` keys = the block's emitter keys and `"fleet_maintenance"` is in neither committed-destination map; (k) the delimiters occur once each, the block parses, every top-level list is non-empty, `len(cases) >= 3`. *Artifacts:* `tests/api/test_story_drift.py`. *Pass read:* eleven green; (g), (i) and (j) print their measured values. *Probes:* P4a–P4l (P4l mutates `sourcing.py` itself, proving the oracle reads the real function). ✅ **Closed 2026-09-16:** (a) `real=10 pinned=10` · (b) `real=7 pinned=7` · (c) 4 undeclared refs · (d) six steps in order, `quote_gate=rule_gate`, `approve=doa_tier`/`gated` · (e) tiers `0 / 5001 / 30001` · (f) SoD `{intake, approve}`, `repair_quote_accepted` · (g) `threshold real=30000 pinned=30000 vendors real=3 pinned=3 ceiling seed=5001.0 pinned=5001` · (h) all three cases equal the real rule · (i) `seed_rows=5 seed_matches=2 illustrative_matches=0` · (j) 7 emitter keys, `outside_tmp=[]` · (k) `delimiters=(1, 1) empties=[] cases=3`. `plan-0126-story-drift.json` is 15/15 WITNESSED, P4l included.
- [x] **AC-5 [check] — no ruled-out literal in the authored story text (lexical, not semantic — §3.6).** Absent: `http://`, `https://`, `www.`, `30,001`, `฿30,001`, `สามหมื่นเอ็ด`, `immutable`, `ไม่ให้โมเดลตัดสินอะไรเลย`, `12/12`; present: `tamper-evident`; `30001` count equals the tier count at that minimum (printed); `AI` (word-bounded) ≤ 1 (printed); a fixture holding `฿30,001` is flagged by the same function. *Artifacts:* `tests/api/test_story_page.py::test_story_text_carries_no_ruled_out_literal`, `tests/api/test_story_page.py::test_the_literal_checker_flags_a_known_numeral`. *Pass read:* both green; prints `ai_tokens=0 band_numeral_in_data=1`. *Probes:* P5a, P5b, P5c. ✅ **Closed 2026-09-16:** `ruled_out_found=[] band_numeral_in_text=1 band_tiers=1 ai_tokens=0 tamper_evident=2`. The control flagged `['30,001', '฿30,001']`. P5a–P5e WITNESSED.
- [x] **AC-6 [check] — story references and story files are a bijection (Trap 3 closed for `story/`).** Forward: every reference in `index.html`, `story.js`, `story.css` resolves on disk; reverse: every `.js`/`.css` in `story/` is referenced or in `_UNREFERENCED_STORY_FILES` (empty); no stale exemption; the parse found ≥ 1 `.js` and ≥ 1 `.css` reference and the directory is non-empty. *Artifacts:* `tests/api/test_story_page.py::test_every_story_reference_resolves`, `tests/api/test_story_page.py::test_every_story_file_is_referenced_or_exempt`, `tests/api/test_story_page.py::test_the_story_parse_is_not_vacuous`. *Pass read:* all green; prints `refs_js=N refs_css=M files=K`. *Probes:* P6a, P6b. ✅ **Closed 2026-09-16:** `refs=9 dangling=[] escaping=[]` · `shipped_js_css=4 orphans=[] stale_exemptions=[]` · `refs_js=3 refs_css=1 files=6`. P6a–P6f WITNESSED. The two directory-shape asserts are exempt in writing, because the driver cannot create or delete directory entries.
- [x] **AC-7 [check] — the console offers the link, it opens a new tab, and no tab was added.** The chosen module (SD-2) renders an anchor with `href: '/story/'`, `target: '_blank'`, `rel: 'noopener'` (parsed with comments stripped); the `ALL_VIEWS` census still equals `MEASURED_TAB_CENSUS` (imported from `test_static_ui.py`, so the control is the existing pin). *Artifacts:* `tests/api/test_story_page.py::test_console_offers_the_story_link_in_a_new_tab`, `tests/api/test_story_page.py::test_the_story_link_added_no_tab`. *Pass read:* both green. *Probes:* P7a–P7f. ✎ *Amended at closeout under option (C), approved by Cray, typed 2026-09-16 (§4.1 closeout amendment). The ratified artifact list named the census test in the static-UI module, which is outside this PLAN's batteries (`was an error (plan)`). The same pin is witnessed through `test_the_story_link_added_no_tab`, which imports `MEASURED_TAB_CENSUS` (P7f). ✎ Also unrecorded until now: the link renders only where the server-declared view set holds both I and H (confirmed by Cray, typed 2026-09-15; rulings file §2.3 L1; commit `456b3b5`), and the first test asserts that gate (P7e).* ✅ **Closed 2026-09-16:** `tag_call="h('a',"` with `target: '_blank', rel: 'noopener'` and `gated_on_I_and_H=True`; `registered=['A'…'J']` equals the census; P7a–P7f WITNESSED. Live: `/` serves `assets/app.js?v=c52` and renders one "ทำไม" anchor with `target=_blank rel=noopener` (AC-10's post-read).
- [x] **AC-8 [check] — scenario: a published visitor reaches the story through the real mount, and the data they receive is the real rules' data (CLAUDE.md §8).** With the published profile patched and no stub anywhere: `GET /` → 200 carrying the `app.js` reference; `GET /story/` → 200 with the CSP header; every reference parsed from the **served** `index.html` and `story.js` → 200 from the same mount; the block extracted from the **served** `story-data.js` body (not from disk) re-passes AC-4(h) against `compute_three_quote`; the served `three.module.min.js` body exceeds 100,000 bytes (a real library, printed). *Artifacts:* `tests/api/test_story_scenario.py::test_a_published_visitor_reaches_the_story_and_its_data_is_the_real_rules`. *Pass read:* green; prints `served_refs=N three_bytes=B cases_checked=3`. *Probes:* P8a, P8b. ✅ **Closed 2026-09-16:** `served_refs=9 failed={} three_bytes=670681 cases_checked=3`; `plan-0126-story-scenario.json` 10/10 WITNESSED.
- [x] **AC-9 [evidence] — Cray's six explainer rulings and the R8-adjacent note are recorded in the rulings file as a record, not a rule.** *Artifact:* `docs/strategy/public/intro-video-production-rulings.md` (a new §2.3 and a note in §4.1 — SD-6). *Pass read:* `git grep -c` over the file prints `1` for the new R8 note's fixed phrase, `1` for the known control phrase `Drop the ฿15,000 contrast`, and `0` for a deliberately misspelt phrase; all three values printed in the PR body. *Probe:* the misspelt-phrase `0` is the negative control; the known phrase `1` the positive. ✅ **Closed 2026-09-16, re-measured at closeout with `git grep -c -F` on HEAD:** `note_phrase=1` (the dated §4.1 note *"2026-09-15: the ฿15,000 contrast returns in the explainer's Act 4 as a third case (L5)"*), `control_phrase=1`, `misspelt_phrase=0`.
- [x] **AC-10 [evidence] — `/story/` is live through the published edge, under Access, with the CSP header.** After Step 13, an authenticated read of `/story/` returns 200 and the CSP header; the same read taken **before** the deploy returned the edge's 404. *Pass read:* the PR body prints `pre=404 post=200 csp=present`, the demo-state token read after boot (`DEMO-STATE: PRISTINE` — no token is FAILED), and the Cray go it ran under. *Probe:* the pre-deploy 404 is the RED baseline; a post-deploy read that omits the CSP header is a FAIL, not a partial pass. ✅ **Closed 2026-09-16:** **`pre=404 post=200 csp=present`**, taken as same-origin reads through the edge after Cray completed the Access login. The CSP on `/story/` and on every story file equals the console's. Controls: `/` returned 200 before and after; `/story` and `/story/sub/x.js` returned 404 after. `DEMO-STATE: PRISTINE` after boot. The go: *"ทำ Step 9 (Artifact), Step 13 deploy และ closeout ตามลำดับได้เลย"* and *"(ก) go ได้เลยเมื่อถึงขั้นตอนที่ต้องการ"*. Record: `docs/logs/2026-09-16-plan0126-fleet-story-deploy.md`.
- [x] **AC-11 [check] — every story reference is relative and resolves inside `story/` or `assets/fonts/`, so the multi-file Artifact's files map is derivable from the files themselves (SD-5 (d)) — plus the publish record as evidence.** Check: over the authored story files (comments stripped, vendor excluded), every `src`, `href`, `import … from`, `@import` and `url(` reference is **relative** (no scheme, no leading `/`) and resolves to a file under `story/` or `assets/fonts/`; a fixture holding a root-relative `/assets/fonts/x.woff2` is shown to be flagged. Evidence: the one publish record printing `files=N sha_match=N rendered=yes|no`, where `sha_match` counts files whose `git hash-object` blob id over the published source copy equals `git rev-parse <merged-sha>:<repo path>`, and `rendered` is Step 9.3's browser read. *Artifacts:* `tests/api/test_story_page.py::test_every_story_reference_is_relative_and_in_bounds`, `tests/api/test_story_page.py::test_the_relative_checker_flags_a_root_relative_url`. *Pass read:* both green; the first prints `refs=N relative=N`; the publish record is printed in the PR body with `sha_match == files` and `rendered=yes`. *Probes:* P11a, P11b. ✅ **Closed 2026-09-16:** `refs=9 relative=9 out_of_bounds=[]`; control `flagged=['/assets/fonts/x.woff2']`; P11a–P11d WITNESSED (#1500). Publish record (in #1500's body and Step 9.6): `files=11 sha_match=11 rendered=yes`.
...
> ✎ **Closeout amendment, 2026-09-16 — rows P3a, P3b and AC-7's census artifact: `was an error (plan)`.**
>
> **What was wrong.** P3a and P3b named nodes in `tests/deploy/test_published_profiles.py`, and AC-7 named `tests/api/test_static_ui.py`. Both modules sit outside the `claim_sources` of this PLAN's own **Batteries:** glob. `tools/check_ac_consistency.py` Check 3 therefore could never have accepted a tick resting on them.
>
> **What Step 11 shipped instead.** Session 304 witnessed AC-3 through story-page probes (P3c–P3f, P7h) and AC-7 through P7a–P7h. That left two halves of AC-3 without a witness:
> - *"the table says so"*: no story test read `_EXPECTED_ALLOW`;
> - a lost **leading** `^`: the admission test matches with cloudflared's unanchored `re.search`, so it stays green. Measured on 2026-09-16 over the real rows with the sibling row's `^` dropped: 6/6 files are still admitted and `/story/sub/x.js` is still refused, while `/evil/story/x.js` would be admitted at the edge.
>
> The PLAN-0125 session found both gaps in a read-only review.
>
> **The fix.** Cray approved option (C), typed 2026-09-16: *"อนุมัติ (C) แล้ว merge #1500 กับ #1496 ให้แล้ว"*.
> - New test: `test_story_page.py::test_the_story_rows_are_anchored_and_the_expected_table_carries_them`. Its anchoring assert runs first, then `table_rows == config_rows == _STORY_ROWS`.
> - **P3a′** (battery name `P3a2`) drops the page row from `_EXPECTED_ALLOW` only. The anchoring assert stays green and the table assert reddens.
> - **P3b′** (battery name `P3b2`) drops the leading `^` from the config's sibling row. The anchoring assert reddens, and the admission test is the sibling that stays green.
> - AC-3's and AC-7's artifacts now name story-page nodes only. Each AC line keeps the ratified list as lineage.
> - The P3a/P3b rows above are kept as drafted.
>
> **Probes the table never planned.** The shipped batteries also carry P-A1, P1c, P2d–P2h, P3d–P3f, P5d, P5e, P6c–P6f, P7c–P7h, P11c, P11d, P4-proc, P4-ceil, P4c0, P8-root, P8-appjs, P8-link, P8-page, P8-csp, P8-set, P8-three and P8-count. **The battery files are the authoritative list.**
...
2. Place it at `services/api/static/story/three.module.min.js` (SD-1 default; amended below) with `THREE_LICENSE.txt` beside it. Never `.mjs` (G11).
3. Record the download (source, sha256, bytes) in the commit body.
4. **No font download** (SD-4 ruled (b)).
5. ✎ **Amendment, 2026-09-15 — the minified build, `superseded by new info`.** Item 1 named `three.module.js`. Cray's first go (typed: *"ให้ดาวน์โหลด แล้วทำ Step 1 ต่อได้เลย"*) was for that file (1,272,972 bytes). It was downloaded and verified, then **not committed**, because the repo's `check-added-large-files` hook (`--maxkb=1000`, `.pre-commit-config.yaml`) rejects it. This was witnessed on a really staged file: *"three.module.js (1244 KB) exceeds 1000 KB"*. A staged 1 KB control passed. An intent-to-add dry run had passed vacuously, because the hook reads only staged additions. Two options went to Cray: (A) the minified build of the same release, or (B) a per-file hook exception in a config file another session owns. Cray ruled (A), typed 2026-09-15: *"เลือก (A) ให้ดาวน์โหลด three.module.min.js ได้เลย"*. Shipped on `feat/plan0126-story-explainer`: `three.module.min.js`, 670,681 bytes, sha256 `3e690ac7…f69ba6`, matching jsdelivr's package metadata; `THREE_LICENSE.txt`, 1,081 bytes, sha256 `852e0e86…bff271d`; both `100644`. Every later reference in this PLAN to the vendored module now names `three.module.min.js`. Outside this amendment, the only remaining `three.module.js` mentions are the stage-1 prototype's CDN import (G26) and item 1's original wording, both kept as history.

### Step 2 — Port the page (SD-3 = (b), SD-4 = (b), SD-5 = (d) — unblocked)

1. Split the prototype: `story.css` (the `<style>` block + `@font-face` for the shipped Plex `.woff2` via **relative** `url('../assets/fonts/…')` — §3.1; every other reference relative too, for AC-11), `story-data.js` (the block, §3.2, mirroring `trace-kinds.js`'s IIFE + delimiter shape), `story.js` (an ES module; `import * as THREE from './three.module.min.js?v=160'`), `index.html` (`<!DOCTYPE html>`, `<html lang="th">`, charset + viewport metas, the chrome, `?v=` tokens on every reference).
...
### Step 9 — Verify the multi-file Artifact publishes from the same files, then publish on request (SD-5 ruled (d), confirmed)

1. **The files map**, derived from AC-11's test rather than typed by hand: every authored file in `story/` plus `three.module.min.js` and `THREE_LICENSE.txt` at their `story/`-relative keys, and each `assets/fonts/*.woff2` that `story.css` references at the key `assets/fonts/<name>`. Root-relative paths are not served by the Artifact host (G32), which is why AC-11 requires every reference to be relative.
...
6. ✎ **Done, 2026-09-16** (Cray's request, typed: *"ทำ Step 9 (Artifact), Step 13 deploy และ closeout ตามลำดับได้เลย"*).
   - **AC-11.** The test and its control landed in #1500. `refs=9 relative=9 out_of_bounds=[]`. The derived files map has 9 keys; with the page and `THREE_LICENSE.txt` it makes 11 published files.
   - **Publish.** One private multi-file Artifact, published from the shared checkout at merged sha `ea6944fa`, with no inlining or rewriting. The record reads `files=11 sha_match=11` (raw-byte `git hash-object --no-filters` against `ea6944fa:<path>`, with a wrong-pairing control).
   - **Host read-back.** 10/10 non-page files are byte-identical. The host wraps `index.html` in its own skeleton (+355 B head, +15 B tail), and the merged page sits inside it unchanged.
   - **9.3, all three properties.**
     - A dev-box replay of the host copies: the wrapped document renders, `./three.module.min.js?v=160` loads and the scene draws, and the relative fonts resolve (4 faces loaded).
     - The host itself: Cray opened the published Artifact in Chrome. It played to `1:56 / 1:56` with 7 acts and the mono chrome. ⇒ **`rendered=yes`**.
   - **No fallback was needed.** Option (a) and the thin wrapper stay unbuilt.
...
`feat/plan0126-story-explainer` → PR with `--body-file`; body carries every printed value from §4 and the Step 6 widths, the Step 2.5 caption list, the Step 1 download record, and the Step 9.3 verification reading for SD-5 (d). **No `--auto`.** Report green; Cray decides when it lands.
...
7. ✎ **Done, 2026-09-16.** The full record is `docs/logs/2026-09-16-plan0126-fleet-story-deploy.md`.
   - **The go**, typed by Cray before any host command: *"ทำ Step 9 (Artifact), Step 13 deploy และ closeout ตามลำดับได้เลย"*, then *"(ก) go ได้เลยเมื่อถึงขั้นตอนที่ต้องการ"*.
   - **§2 pre-flight:** `DEMO-STATE: PRISTINE`. The host checkout was at `dd4228f` and clean.
   - **§2a pre-ship:** a build from `git archive ea6944fa`. All 33 of the COPY-carried files changed since `dd4228f` hash identically inside the image, and a local published-profile boot served `/story/` 200 with CSP.
   - **AC-10 pre-read:** `/story/` returned **404** through the edge. Cray completed the Access login.
   - **§3 ship:** the auto-mode classifier refused Code's run of the ship script, so **Cray ran it from their own terminal**. Code verified every step's token. `:prev` equals the baseline; the image id is equal across machines; the host `pull --ff-only` landed on `ea6944fa`; `config --quiet` printed 0 bytes; only `app` was recreated, plus the required `--force-recreate cloudflared`.
   - **§4 verify:** healthy on the new image. Postgres and the sibling systems' containers are unchanged. The seeded run survived, the state is still `PRISTINE`, and the running container's hashes match 8/8.
   - **AC-10 post-read:** **200 + CSP**; the controls `/story` and `/story/sub/x.js` return 404 at the edge.
   - ⚠️ The deploy carried all of `main` since `dd4228f`, not only the story.
...
✎ **Closeout order, 2026-09-16** (reviewed by the PLAN-0125 session):
1. **The ticks land in the closeout PR, together with the evidence they rest on.** Check 3 reads active PLANs only, so ticking after the `git mv` would never be checked.
2. Cray ratifies completion.
3. The `git mv` goes in its own `docs/*` PR.
4. The PLAN-0125 session reconciles STATUS. It cites PLAN-0126 by number, not by path, so the citation survives the move.
...
- **Recommendation: (a).** PLAN-0100 wrote `endswith("index.html")` when exactly one index existed; narrowing states the guarantee it actually meant, and the accelerator clause covers this — the serving tests run against the real mount under `TestClient`. Sub-choice folded in: the vendored module lives **story-local** (`story/three.module.min.js`) so AC-6 guards it and no `_UNREFERENCED_ASSETS` row is needed (a top-level `assets/three.module.min.js` would be a manifest orphan; an `assets/vendor/` subdir would be unguarded — G6).
...
- **RULED (d) — confirmed.** Code's review recommended (d) over the drafter's (a), and Cray's typed *"เอาตามที่แนะนำทั้งหมด"* (2026-09-15) was first recorded as an **interpretation** endorsing that recommendation. ✎ **Confirmed at PR #1495 review — Cray, typed 2026-09-15: *"SD-5 ยืนยัน (d)"*.** The interpretation is kept above for lineage; the ruling no longer rests on it. Step 9.3 verifies (d)'s three asserted-not-verified host properties before the first publish; if any fails, the fallback to (a) or a thin wrapper comes back to Cray as a new SD. The drafter's (a) recommendation above is kept as written for lineage.
