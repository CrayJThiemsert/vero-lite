...
**Where new knowledge goes** (the ADR-0017 D5 routing rule): a **binding rule** the agent must always obey → `CLAUDE.md` (keep it short); a **durable learning** → `docs/lessons/` (advisory); a **canonical reference / standard** you look up deliberately → `docs/conventions/` (or `docs/runbooks/` for operational how-to); a **task-triggered procedure** best surfaced automatically at the moment of need → a **Skill** (`.claude/skills/`). Bright line: a binding rule never moves into a skill (a skill that fails to trigger would silently drop it) — `CLAUDE.md` holds the rule, the skill holds the how-to. Same test, generalised: **name the rule's consumer, then check the home is in that consumer's input.** A rule meant to bind an automated judge or enforcer — a classifier, a hook, the `goal-evaluator` — must live in that enforcer's own input surface; written anywhere else it is, for that consumer, not written. A gate or tripwire — its definition *and* its do-not-act instruction — must live on a tracked, scanned surface; a gitignored handoff or an archived PLAN holds the rationale, never the gate. Full rule + skill-authoring conventions: [`docs/runbooks/memory-architecture.md`](docs/runbooks/memory-architecture.md).
...
- **An inherited premise a decision rests on is a claim, not context** — a fact-pack line, an SD slot's premise, a STATUS shorthand, a remembered baseline. Before the decision consumes it, re-measure it against the live surface or mark it `asserted-not-verified` where the decision is recorded — strictest for negative and precondition claims ("X is unbuilt / absent / deferred").
...
- **A load-bearing green — an AC-closing test, a guard, a measurement a decision rests on — is not evidence until its assertion has been witnessed RED:** a mutation of what the assertion is about, shown to redden *that assertion*, in the direction it claims. An added payload cannot vouch for a presence claim, nor a removal for an absence one; a zero or absence needs a positive control that finds a known one. **One mutation witnesses only ONE assertion** — the run stops at the first failed assert — so a pass read naming two reddening assertions needs **one probe per assertion**, each preferring a mutation under which the *other* assertion stays green: that green is the only thing ruling out "something unrelated broke". A negative assertion carries its own positive control or it is vacuous — "absent from the list" is satisfied by an empty list. **If an instrument disagrees with the artifact, suspect the INSTRUMENT first** — go read the artifact, then repair the check by *deriving* the correct expectation, never by relaxing the one that just failed. This is **not scoped to probe batteries**: it fired seven times in s269 across a byte-delta criterion, a verification regex, two merge-verification tokens and a positive control, and the artifact was right **every** time. Two preventive halves, because the silent failures are the costly ones: **an instrument whose reading you will act on passes a control on known content before its first real reading is trusted** (an uncontrolled instrument does not fail loudly, it fails *confidently*), and **count the actual variety before coding against one instance** (a shape read off one example works perfectly on that example). A verification report prints the values it measured — `pre=1 post=2`, never a bare PASS/FAIL; that printed value is what makes a disagreement one step to diagnose instead of a hunt. Worked cases: [`docs/lessons/0056-suspect-the-instrument-before-the-artifact.md`](docs/lessons/0056-suspect-the-instrument-before-the-artifact.md). Probe batteries run through the shipped driver — **`tools/probe_battery/`** — never a from-scratch `/tmp` script: crash-vs-assertion crediting, one-claim-per-probe, `Claim.stable_key` addressing, SIGTERM-surviving restore, and the mandatory coverage report live in the tool, and a hand-rolled driver was measured (s253) re-making all four of its retired defect classes at once. Mechanics: the module README (PLAN-0115).
- **An expected-value set — a gold set, benchmark baseline, golden snapshot — is not an oracle of the system until the system's own output is scored against it.** Well-formedness checks on such a file (a count agreeing with its own id list) are legitimate and stay, but they compare the file to itself: they redden on a malformed file and cannot redden on a wrong value, so they satisfy the rule above and still close nothing about the system. A case the system cannot express is registered as inexpressible, in writing, with its reason.
