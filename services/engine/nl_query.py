...
def _translate_messages(
    ...
    system = (
        ...
        "OPERATION: use 'count' for how-many questions — and ALSO set group_by when the "
        "question asks how many PER something (e.g. 'how many events per asset?' -> "
        "operation 'count', group_by 'asset_id', no aggregate_property); use "
        "'max'/'min'/'avg'/'sum' for aggregate questions (highest/lowest/average/total of a "
        "numeric property — set aggregate_property to that property, and set group_by when "
        "the question asks 'per <thing>' or 'which <entity>'); otherwise 'list'. If you set "
        "aggregate_property, operation must be max/min/avg/sum. Never combine 'list' with "
        "aggregate_property or group_by.\n"
